package com.earthspay.lang.compiler
import cats.kernel.Monoid
import com.earthspay.common.state.ByteStr
import com.earthspay.lang.Common.{NoShrink, produce}
import com.earthspay.lang.contract.Contract
import com.earthspay.lang.contract.Contract.{CallableAnnotation, CallableFunction, VerifierAnnotation, VerifierFunction}
import com.earthspay.lang.v1.FunctionHeader.{Native, User}
import com.earthspay.lang.v1.compiler
import com.earthspay.lang.v1.compiler.Terms
import com.earthspay.lang.v1.compiler.Terms._
import com.earthspay.lang.v1.evaluator.FunctionIds
import com.earthspay.lang.v1.evaluator.ctx.impl.earths.{FieldNames, EarthsContext}
...
import com.earthspay.lang.{Common, StdLibVersion}
...
class ContractCompilerTest extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  property("contract compiles when uses annotation bindings and correct return type") {
    val ctx = Monoid.combine(compilerContext,
                             EarthsContext.build(StdLibVersion.V3, Common.emptyBlockchainEnvironment(), isTokenContext = false).compilerContext)
    val expr = {
      val script =
        """
          |
  ...
  property("contract compiles callable functions independently") {
    val ctx = Monoid.combine(compilerContext, EarthsContext.build(StdLibVersion.V3, Common.emptyBlockchainEnvironment(), false).compilerContext)
    val expr = {
      val script =
        """
          |
    ...
    val ctx = Monoid
      .combineAll(
        Seq(
          ...
          CryptoContext.build(com.earthspay.lang.Global),
    ...
    val expr = {
      val script =
        """
          ...
          |   if (isDefined(pmt.asset)) then throw("can hodl earths only at the moment")
          |   else {
    ...
    val expr = {
      val script =
        """
          ...
          |   else TransferSet(List(ContractTransfer(i.caller, earthsBalance(i.contractAddress), base58'somestr')))
          | }
          |
          | @Verifier(t)
          | func verify() = {
