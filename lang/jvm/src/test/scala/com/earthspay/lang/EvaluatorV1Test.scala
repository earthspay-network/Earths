...
import com.earthspay.lang.v1.evaluator.ctx.impl.{CryptoContext, EnvironmentFunctions, PureContext, _}
import com.earthspay.lang.v1.evaluator.{EvaluatorV1, Log}
...
class EvaluatorV1Test extends PropSpec with PropertyChecks with Matchers with ScriptGen with NoShrink {
  ...
  private def defaultFullContext(environment: Environment): CTX = Monoid.combineAll(
    Seq(
      ...
      pureContext,
      EarthsContext.build(V1, environment, isTokenContext = false)
    )
  )
