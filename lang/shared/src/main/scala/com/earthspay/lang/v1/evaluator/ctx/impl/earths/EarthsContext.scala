...
import com.earthspay.lang.v1.compiler.Types.{BYTESTR, LONG, STRING, _}
  ...
  def build(version: StdLibVersion, env: Environment, isTokenContext: Boolean): CTX = {
    ...
    val assetBalanceF: BaseFunction =
      NativeFunction(
        ...
        ("addressOrAlias", addressOrAliasType, "account"),
        ("assetId", UNION(UNIT, BYTESTR), "assetId (EARTHS if none)")
      ) {
    ...
    val earthsBalanceF: UserFunction =
      UserFunction("earthsBalance", 109, LONG, "get EARTHS balanse for account", ("@addressOrAlias", addressOrAliasType, "account")) {
    ...
    val types = buildEarthsTypes(proofsEnabled, version)
