...
import com.earthspay.transaction.ValidationError.{GenericError, OrderValidationError}
...
object ExchangeTransactionDiff {
  ...
  def apply(blockchain: Blockchain, height: Int)(tx: ExchangeTransaction): Either[ValidationError, Diff] = {
    ...
    for {
      ...
      def earthsPortfolio(amt: Long) = Portfolio(amt, LeaseBalance.empty, Map.empty)

      val feeDiff = Monoid.combineAll(
        Seq(
          Map(matcher -> earthsPortfolio(t.buyMatcherFee + t.sellMatcherFee - t.fee)),
          Map(buyer   -> earthsPortfolio(-t.buyMatcherFee)),
          Map(seller  -> earthsPortfolio(-t.sellMatcherFee))
        ))
