...
import com.earthspay.transaction.ValidationError.{AliasDoesNotExist, AliasIsDisabled}
...
class CompositeBlockchain(inner: Blockchain, maybeDiff: => Option[Diff], carry: Long = 0) extends Blockchain {
  ...
  override def earthsDistribution(height: Int): Map[Address, Long] = {
    val innerDistribution = inner.earthsDistribution(height)
