...
import com.earthspay.account.{Address, Alias, PublicKeyAccount}
...
object Sponsorship {
  ...
  def toEarths(assetFee: Long, sponsorship: Long): Long = {
    val earths = (BigDecimal(assetFee) * BigDecimal(CommonValidation.FeeUnit)) / BigDecimal(sponsorship)
    if (earths > Long.MaxValue) {
      throw new java.lang.ArithmeticException("Overflow")
    }
    earths.toLong
  }

  def fromEarths(earthsFee: Long, sponsorship: Long): Long = {
    val assetFee = (BigDecimal(earthsFee) / BigDecimal(CommonValidation.FeeUnit)) * BigDecimal(sponsorship)
