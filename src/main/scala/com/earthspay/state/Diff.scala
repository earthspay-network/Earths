...
object Sponsorship {
  ...
  def toEarths(assetFee: Long, sponsorship: Long): Long = {
    val earths = (BigDecimal(assetFee) * BigDecimal(Sponsorship.FeeUnit)) / BigDecimal(sponsorship)
  ...
  def fromEarths(earthsFee: Long, sponsorship: Long): Long = {
    val assetFee = (BigDecimal(earthsFee) / BigDecimal(Sponsorship.FeeUnit)) * BigDecimal(sponsorship)
