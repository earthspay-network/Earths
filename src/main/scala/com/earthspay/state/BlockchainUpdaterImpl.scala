...
import com.earthspay.block.{Block, BlockHeader, MicroBlock}
import com.earthspay.features.BlockchainFeatures
import com.earthspay.features.FeatureProvider._
import com.earthspay.metrics.{Instrumented, TxsInBlockchainStats}
import com.earthspay.mining.{MiningConstraint, MiningConstraints, MultiDimensionalMiningConstraint}
import com.earthspay.settings.EarthsSettings
import com.earthspay.state.diffs.BlockDiffer
import com.earthspay.state.reader.{CompositeBlockchain, LeaseDetails}
import com.earthspay.transaction.Transaction.Type
import com.earthspay.transaction.ValidationError.{BlockAppendError, GenericError, MicroBlockAppendError}
...
import com.earthspay.utils.{ScorexLogging, Time, UnsupportedFeature, forceStopApplication}
import kamon.Kamon
import kamon.metric.MeasurementUnit
import monix.reactive.Observable
import monix.reactive.subjects.ConcurrentSubject

class BlockchainUpdaterImpl(blockchain: Blockchain, settings: EarthsSettings, time: Time)
    extends BlockchainUpdater
    with NG
    with ScorexLogging
    with Instrumented {

  import com.earthspay.state.BlockchainUpdaterImpl._
  ...
  /** Retrieves Earths balance snapshot in the [from, to] range (inclusive) */
  ...
  override def earthsDistribution(height: Int): Map[Address, Long] = ngState.fold(blockchain.earthsDistribution(height)) { ng =>
    val innerDistribution = blockchain.earthsDistribution(height)
  ...
  /** Builds a new portfolio map by applying a partial function to all portfolios on which the function is defined.
    *
    * @note Portfolios passed to `pf` only contain Earths and Leasing balances to improve performance */
