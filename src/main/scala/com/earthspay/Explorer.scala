...
import com.earthspay.database.{Keys, LevelDBWriter}
import com.earthspay.db.openDB
import com.earthspay.settings.{EarthsSettings, loadConfig}
import com.earthspay.state.{Height, TxNum}
...
object Explorer extends ScorexLogging {
  ...
  private val keys = Array(
    ...
    "sponsorship-history",
    "sponsorship",
    "addresses-for-earths-seq-nr",
    "addresses-for-earths",
  ...
  def main(args: Array[String]): Unit = {
    ...
    val settings = EarthsSettings.fromConfig(loadConfig(ConfigFactory.parseFile(new File(configFilename))))
      ...
      flag match {
        ...
        case "A" =>
          ...
          val balances = wbh.map { h =>
            val k = Keys.earthsBalance(addressId)(h)
