...
import com.earthspay.Shutdownable
import com.earthspay.settings.{Constants, RestAPISettings}
import com.earthspay.state.Blockchain
import io.swagger.annotations._
import javax.ws.rs.Path
import play.api.libs.json.Json
import com.earthspay.api.http.{ApiRoute, CommonApiFunctions}
...
case class NodeApiRoute(settings: RestAPISettings, blockchain: Blockchain, application: Shutdownable)
  ...
  @Path("/version")
  @ApiOperation(value = "Version", notes = "Get Earths node version", httpMethod = "GET")
  @ApiResponses(
    Array(
      new ApiResponse(code = 200, message = "Json Earths node version")
    ))
