...
import com.earthspay.mining.{Miner, MinerDebugInfo}
import com.earthspay.network.{LocalScoreChanged, PeerDatabase, PeerInfo, _}
import com.earthspay.settings.EarthsSettings
import com.earthspay.state.diffs.TransactionDiffer
import com.earthspay.state.{Blockchain, ByteStr, LeaseBalance, NG, Portfolio}
import com.earthspay.transaction.ValidationError.InvalidRequestSignature
import com.earthspay.transaction._
import com.earthspay.transaction.smart.Verifier
import com.earthspay.utils.{Base58, ScorexLogging, Time}
...
@Path("/debug")
@Api(value = "/debug")
case class DebugApiRoute(ws: EarthsSettings,
                         time: Time,
  ...
  private lazy val earthsConfig: JsObject = Json.obj("earths" -> (fullConfig \ "earths").get)

  override val settings = ws.restAPISettings
  override lazy val route: Route = pathPrefix("debug") {
    blocks ~ state ~ info ~ stateEarths ~ rollback ~ rollbackTo ~ blacklist ~ portfolios ~ minerInfo ~ historyInfo ~ configInfo ~ print ~ validate
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.earthspay.http.DebugMessage",
        defaultValue = "{\n\t\"message\": \"foo\"\n}"
      )
    ))
  ...
  def state: Route = (path("state") & get & withAuth) {
    complete(ng.earthsDistribution(ng.height).map { case (a, b) => a.stringRepr -> b })
  }

  @Path("/stateEarths/{height}")
  @ApiOperation(value = "State at block", notes = "Get state at specified height", httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "height", value = "height", required = true, dataType = "integer", paramType = "path")
    ))
  def stateEarths: Route = (path("stateEarths" / IntNumber) & get & withAuth) { height =>
    complete(ng.earthsDistribution(height).map { case (a, b) => a.stringRepr -> b })
  }
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
        ...
        required = true,
        paramType = "body",
        dataType = "com.earthspay.http.RollbackParams",
  ...
  def configInfo: Route = (path("configInfo") & get & parameter('full.as[Boolean]) & withAuth) { full =>
    complete(if (full) fullConfig else earthsConfig)
  }
