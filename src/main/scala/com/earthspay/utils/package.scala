...
import com.earthspay.lang.v1.{FunctionHeader, ScriptEstimator}
import com.earthspay.transaction.smart.{BlockchainContext, EarthsEnvironment}
  ...
  lazy val dummyCompilerContext: CompilerContext =
    Monoid.combineAll(
      Seq(
        ...
        EarthsContext.build(new EarthsEnvironment(dummyNetworkByte, Coeval(???), Coeval(???), null)).compilerContext,
