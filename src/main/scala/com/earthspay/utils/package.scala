...
import com.earthspay.common.state.ByteStr._
import com.earthspay.common.utils.EitherExt2
import com.earthspay.db.{Storage, VersionedStorage}
import com.earthspay.lang.Global
import com.earthspay.lang.StdLibVersion._
import com.earthspay.lang.v1.compiler.{CompilerContext, DecompilerContext}
...
import com.earthspay.lang.v1.{CTX, FunctionHeader, ScriptEstimator}
...
package object utils extends ScorexLogging {
  ...
  def forceStopApplication(reason: ApplicationStopReason = Default): Unit =
    ...
    }, "earths-platform-shutdown-thread").start()
  ...
  private val lazyAssetContexts: Map[StdLibVersion, Coeval[CTX]] =
    Seq
      .tabulate(2) { v =>
        val version = com.earthspay.lang.StdLibVersion.parseVersion(v + 1)
        version -> Coeval.evalOnce(
          Monoid
            .combineAll(Seq(
              ...
              EarthsContext
                .build(version, new EarthsEnvironment(AddressScheme.current.chainId, Coeval(???), Coeval(???), EmptyBlockchain), isTokenContext = true)
            )))
      }
      .toMap
  ...
  private val lazyContexts: Map[StdLibVersion, Coeval[CTX]] =
    Seq
      .tabulate(3) { v =>
        val version: StdLibVersion = com.earthspay.lang.StdLibVersion(v + 1)
        version -> Coeval.evalOnce(
          Monoid
            .combineAll(Seq(
              PureContext.build(version),
              CryptoContext.build(Global),
              EarthsContext.build(version,
                                 new EarthsEnvironment(AddressScheme.current.chainId, Coeval(???), Coeval(???), EmptyBlockchain),
