...
import com.earthspay.actor.RootActorSystem
import com.earthspay.api.http._
import com.earthspay.api.http.alias.{AliasApiRoute, AliasBroadcastApiRoute}
import com.earthspay.api.http.assets.{AssetsApiRoute, AssetsBroadcastApiRoute}
import com.earthspay.api.http.leasing.{LeaseApiRoute, LeaseBroadcastApiRoute}
...
import com.earthspay.features.api.ActivationApiRoute
import com.earthspay.history.{CheckpointServiceImpl, StorageFactory}
import com.earthspay.http.{DebugApiRoute, NodeApiRoute, EarthsApiRoute}
import com.earthspay.matcher.Matcher
import com.earthspay.metrics.Metrics
import com.earthspay.mining.{Miner, MinerImpl}
import com.earthspay.network.RxExtensionLoader.RxExtensionLoaderShutdownHook
import com.earthspay.network._
import com.earthspay.settings._
import com.earthspay.state.appender.{BlockAppender, CheckpointAppender, ExtensionAppender, MicroblockAppender}
import com.earthspay.transaction._
import com.earthspay.utils.{NTP, ScorexLogging, SystemInformationReporter, Time, forceStopApplication}
import com.earthspay.utx.{MatcherUtxPool, UtxPool, UtxPoolImpl}
...
class Application(val actorSystem: ActorSystem, val settings: EarthsSettings, configRoot: ConfigObject) extends ScorexLogging {
  ...
  def run(): Unit = {
    ...
    if (settings.restAPISettings.enable) {
      val apiRoutes = Seq(
        ...
        EarthsApiRoute(settings.restAPISettings, wallet, utxStorage, allChannels, time),
...
object Application extends ScorexLogging {
    ...
    val config = maybeConfigFile match {
      ...
      case Some(file) =>
        ...
        if (!cfg.hasPath("earths")) {
          ...
          log.error("https://github.com/earthspay/Earths/wiki/Earths-Node-configuration-file")
  ...
  def main(args: Array[String]): Unit = {
    ...
    System.setProperty("earths.directory", config.getString("earths.directory"))
    log.info("Starting...")
    sys.addShutdownHook {
    ...
    RootActorSystem.start("earthspay", config) { actorSystem =>
