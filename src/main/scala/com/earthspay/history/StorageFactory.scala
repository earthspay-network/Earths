...
import com.earthspay.database.{DBExt, Keys, LevelDBWriter}
import com.earthspay.settings.EarthsSettings
import com.earthspay.state.{BlockchainUpdaterImpl, NG}
import com.earthspay.transaction.{AssetId, BlockchainUpdater}
...
object StorageFactory extends ScorexLogging {
  ...
  def apply(settings: EarthsSettings, db: DB, time: Time, spendableBalanceChanged: Observer[(Address, Option[AssetId])]): BlockchainUpdater with NG = {
    checkVersion(db)
