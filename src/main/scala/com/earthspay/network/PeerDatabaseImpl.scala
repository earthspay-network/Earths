...
import com.earthspay.utils.{JsonFileStorage, ScorexLogging}
...
class PeerDatabaseImpl(settings: NetworkSettings) extends PeerDatabase with ScorexLogging {
  ...
  for (a <- settings.knownPeers.view.map(inetSocketAddress(_, 7773))) {
  ...
  for (f <- settings.file if f.exists()) try {
    JsonFileStorage.load[PeersPersistenceType](f.getCanonicalPath).foreach(a => touch(inetSocketAddress(a, 7773)))
