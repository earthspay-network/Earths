...
import com.earthspay.lang.v1.traits.Recipient.{Address, Alias}
import com.earthspay.lang.v1.traits.{DataType, Environment, Recipient, Tx => ContractTransaction}
...
class EarthsEnvironment(nByte: Byte, tx: Coeval[Transaction], h: Coeval[Int], blockchain: Blockchain) extends Environment {
  ...
  override def resolveAlias(name: String): Either[String, Recipient.Address] =
    blockchain
      .resolveAlias(com.earthspay.account.Alias.buildWithCurrentNetworkByte(name).explicitGet())
      .left
      .map(_.toString)
      .right
  ...
  override def accountBalanceOf(addressOrAlias: Recipient, maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        ...
        case Alias(name)    => com.earthspay.account.Alias.buildWithCurrentNetworkByte(name)
      }
