package com.earthspay.transaction.assets.exchange

import com.earthspay.state.ByteStr
import com.earthspay.transaction._
import com.earthspay.transaction.assets.exchange.Order.assetIdBytes
...
object AssetPair {
  ...
  def assetIdStr(aid: Option[AssetId]): String = aid.fold(EarthsName)(_.base58)

  def extractAssetId(a: String): Try[Option[AssetId]] = a match {
    case `EarthsName` => Success(None)
