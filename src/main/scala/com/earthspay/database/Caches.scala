...
import com.earthspay.metrics.LevelDBStats
...
import com.earthspay.utils.{ObservedLoadingCache, ScorexLogging}
...
abstract class Caches(spendableBalanceChanged: Observer[(Address, Option[AssetId])]) extends Blockchain with ScorexLogging {
  ...
  override def append(diff: Diff, carryFee: Long, block: Block): Unit = {
    ...
    val earthsBalances        = Map.newBuilder[BigInt, Long]
