...
import com.earthspay.block.BlockHeader
...
import com.earthspay.transaction.{Transaction, TransactionParsers}
...
object Keys {
  ...
  def earthsBalanceHistory(addressId: BigInt): Key[Seq[Int]] = historyKey("earths-balance-history", 5, addressId.toByteArray)
  def earthsBalance(addressId: BigInt)(height: Int): Key[Long] =
    Key("earths-balance", hAddr(6, height, addressId), Option(_).fold(0L)(Longs.fromByteArray), Longs.toByteArray)
  ...
  val addressesForEarthsSeqNr: Key[Int]                = intKey("addresses-for-earths-seq-nr", 37)
  def addressesForEarths(seqNr: Int): Key[Seq[BigInt]] = Key("addresses-for-earths", h(38, seqNr), readBigIntSeq, writeBigIntSeq)
