...
import com.earthspay.matcher.MatcherSettings.EventsQueueSettings
import com.earthspay.matcher.api.OrderBookSnapshotHttpCache
import com.earthspay.matcher.queue.{KafkaMatcherQueue, LocalMatcherQueue}
...
class MatcherSettingsSpecification extends FlatSpec with Matchers {
  "MatcherSettings" should "read values" in {
    ...
    settings.port should be(7776)
    settings.minOrderFee should be(100000)
    settings.orderMatchTxFee should be(100000)
    settings.journalDataDir should be("/earths/matcher/journal")
    settings.snapshotsDataDir should be("/earths/matcher/snapshots")
    ...
    settings.priceAssets should be(Seq("EARTHS", "8LQW8f7P5d5PZM7GtZEBgaqRPGSzS3DfPuiXrURJ4AJS", "DHgwrRvVyqJsepd32YbBqUeDH4GJ1N984X8QoekjgH8J"))
