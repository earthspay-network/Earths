...
class BlockchainSettingsSpecification extends FlatSpec with Matchers {
  "BlockchainSettings" should "read custom values" in {
    val config   = loadConfig(ConfigFactory.parseString("""earths {
        ...
        |      }
        |      genesis {
        |        timestamp = 1544864400000
        |        block-timestamp = 1544864400000
    ...
    settings.genesisSettings.blockTimestamp should be(1544864400000L)
    settings.genesisSettings.timestamp should be(1544864400000L)
  ...
  it should "read testnet settings" in {
    ...
    settings.functionalitySettings.allowTemporaryNegativeUntil should be(1544864400000L)
    ...
    settings.functionalitySettings.allowTransactionsFromFutureUntil should be(1544864400000L)
    settings.functionalitySettings.allowUnissuedAssetsUntil should be(1544864400000L)
    settings.functionalitySettings.allowInvalidReissueInSameBlockUntilTimestamp should be(1544864400000L)
    settings.functionalitySettings.allowMultipleLeaseCancelTransactionUntilTimestamp should be(1544864400000L)
    ...
    settings.genesisSettings.signature should be(
      ByteStr.decodeBase58("52He8VmHPtsJV77nmrjzEcvAqpUpTFNRj4hVXoheNNYcmoTXP3nMjKrRX4V9sF66sXxavVVYtGsFsn2b2pvNRD7H").toOption)
  ...
  it should "read mainnet settings" in {
    ...
    settings.functionalitySettings.minimalGeneratingBalanceAfter should be(1544864400000L)
    settings.functionalitySettings.allowTransactionsFromFutureUntil should be(1544864400000L)
    settings.functionalitySettings.allowUnissuedAssetsUntil should be(1544864400000L)
    settings.functionalitySettings.allowInvalidReissueInSameBlockUntilTimestamp should be(1544864400000L)
    settings.functionalitySettings.allowMultipleLeaseCancelTransactionUntilTimestamp should be(1544864400000L)
    ...
    settings.genesisSettings.timestamp should be(1544864400000L)
    settings.genesisSettings.signature should be(
      ByteStr.decodeBase58("VobW8JoGdP2HMrYAp3vP3G41Zp6bUB4Jgx8HdcDukPr8CKEDkZiDyokzR324M68KRd83BG8w83u6mBCd4s3yp5b").toOption)
