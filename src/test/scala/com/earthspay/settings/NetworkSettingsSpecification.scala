...
class NetworkSettingsSpecification extends FlatSpec with Matchers {
  ...
  "NetworkSpecification" should "read values from config" in {
    val config          = loadConfig(ConfigFactory.parseString("""earths.network {
        |  bind-address: "127.0.0.1"
        |  port: 7778
        |  node-name: "default-node-name"
        |  declared-address: "127.0.0.1:7778"
        |  nonce: 0
        |  known-peers = ["8.8.8.8:7778", "4.4.8.8:7778"]
        |  local-only: no
    ...
    networkSettings.bindAddress should be(new InetSocketAddress("127.0.0.1", 7778))
    networkSettings.nodeName should be("default-node-name")
    networkSettings.declaredAddress should be(Some(new InetSocketAddress("127.0.0.1", 7778)))
    networkSettings.nonce should be(0)
    networkSettings.knownPeers should be(List("8.8.8.8:7778", "4.4.8.8:7778"))
  ...
  it should "build node name using nonce" in {
    val config          = loadConfig(ConfigFactory.parseString("earths.network.nonce = 12345"))
    val networkSettings = config.as[NetworkSettings]("earths.network")
  ...
  it should "fail with IllegalArgumentException on too long node name" in {
    val config = loadConfig(ConfigFactory.parseString(
      "earths.network.node-name = очень-длинное-название-в-многобайтной-кодировке-отличной-от-однобайтной-кодировки-американского-института-стандартов"))
    intercept[IllegalArgumentException] {
      config.as[NetworkSettings]("earths.network")
    }
  }
}
