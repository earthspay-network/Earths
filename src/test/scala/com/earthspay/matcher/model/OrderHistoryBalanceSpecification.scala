...
import com.earthspay.matcher.{AddressActor, MatcherTestData}
...
class OrderHistoryBalanceSpecification
  ...
  private val EarthsBtc = AssetPair(None, mkAssetId("BTC"))
  ...
  property("New buy EARTHS order added") {
    val ord = buy(EarthsBtc, 1000, 0.008, matcherFee = Some(3000))
    val lo  = LimitOrder(ord)
    ...
    withClue("reserved assets considering amount of received EARTHS") {
      openVolume(ord.senderPublicKey, EarthsBtc.amountAsset) shouldBe 2000L
      openVolume(ord.senderPublicKey, EarthsBtc.priceAsset) shouldBe 8L
    }

    activeOrderIds(ord.senderPublicKey) shouldBe Seq(ord.id())
  }

  property("New sell EARTHS order added") {
    val ord = sell(EarthsBtc, 10000, 0.0008)
    val lo  = LimitOrder(ord)

    oh.process(OrderAdded(lo))
    orderStatus(ord.id()) shouldBe OrderStatus.Accepted
    openVolume(ord.senderPublicKey, EarthsBtc.amountAsset) shouldBe 10000L + ord.matcherFee
    openVolume(ord.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L

    activeOrderIds(ord.senderPublicKey) shouldBe Seq(ord.id())
  }

  property("Should not reserve fee, if seller receives more EARTHS than total fee in sell order") {
  ...
  property("Should not reserve fee, if buyer receives more EARTHS than total fee in buy order") {
    val ord = buy(EarthsBtc, 100000, 0.0007, matcherFee = Some(1000L))

    oh.process(OrderAdded(LimitOrder(ord)))
    orderStatus(ord.id()) shouldBe OrderStatus.Accepted

    openVolume(ord.senderPublicKey, EarthsBtc.amountAsset) shouldBe 0L
  }

  property("Two sell orders added") {
    val pk   = PrivateKeyAccount("private".getBytes("utf-8"))
    val ord1 = sell(EarthsBtc, 10000, 0.0005, Some(pk), matcherFee = Some(30000L), ts = Some(System.currentTimeMillis()))
    val ord2 = sell(EarthsBtc, 16000, 0.0008, Some(pk), matcherFee = Some(30000L), ts = Some(System.currentTimeMillis() + 1))
    ...
    withClue("correction was used to reserve assets") {
      openVolume(ord1.senderPublicKey, EarthsBtc.amountAsset) shouldBe ord1.amount + ord1.matcherFee + ord2.amount + ord2.matcherFee
      openVolume(ord1.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L
    }
    ...
    withClue("orders list") {
      ...
      activeOrderIdsByPair(ord1.senderPublicKey, EarthsBtc) shouldBe expected
      allOrderIdsByPair(ord1.senderPublicKey, EarthsBtc) shouldBe expected
    }
  }

  property("Buy EARTHS order filled exactly") {
    val counter   = buy(EarthsBtc, 100000, 0.0008, matcherFee = Some(2000L))
    val submitted = sell(EarthsBtc, 100000, 0.0007, matcherFee = Some(1000L))
    ...
    withClue(s"has no reserved assets, counter.senderPublicKey: ${counter.senderPublicKey}, counter.order.id=${counter.idStr()}") {
      openVolume(counter.senderPublicKey, EarthsBtc.amountAsset) shouldBe 0L
      openVolume(counter.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L
    }

    withClue(s"has no reserved assets, submitted.senderPublicKey: ${submitted.senderPublicKey}, submitted.order.id=${submitted.idStr()}") {
      openVolume(submitted.senderPublicKey, EarthsBtc.amountAsset) shouldBe 0L
      openVolume(submitted.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L
    }
    ...
    withClue(s"orders list of counter owner ${counter.sender}") {
      ...
      activeOrderIdsByPair(counter.senderPublicKey, EarthsBtc) shouldBe empty
      allOrderIdsByPair(counter.senderPublicKey, EarthsBtc) shouldBe Seq(counter.id())
    }
    ...
    withClue(s"orders list of submitted owner ${submitted.sender}") {
      ...
      activeOrderIdsByPair(submitted.senderPublicKey, EarthsBtc) shouldBe empty
      allOrderIdsByPair(submitted.senderPublicKey, EarthsBtc) shouldBe Seq(submitted.id())
    }
  }

  property("Buy EARTHS order filled with remainder") {
    val counter   = sell(EarthsBtc, 840340L, 0.00000238, matcherFee = Some(300000L))
    val submitted = buy(EarthsBtc, 425532L, 0.00000238, matcherFee = Some(300000L))
    ...
    withClue("orders list of counter owner") {
      ...
      activeOrderIdsByPair(counter.senderPublicKey, EarthsBtc) shouldBe expected
      allOrderIdsByPair(counter.senderPublicKey, EarthsBtc) shouldBe expected
    }
  ...
  property("Sell EARTHS order - filled, buy order - partial") {
    val counter   = sell(EarthsBtc, 100000000, 0.0008, matcherFee = Some(2000L))
    val submitted = buy(EarthsBtc, 120000000, 0.00085, matcherFee = Some(1000L))
...
//                 OrderInfo.safeSum(LimitOrder.getPartialFee(submitted.matcherFee, submitted.amount, submitted.amount - counter.amount), -20000000L))
      openVolume(submitted.senderPublicKey, EarthsBtc.priceAsset) shouldBe (BigDecimal(0.00085) * 20000000L).toLong
    }
    ...
    withClue("orders list of submitted owner") {
      ...
      activeOrderIdsByPair(submitted.senderPublicKey, EarthsBtc) shouldBe expected
      allOrderIdsByPair(submitted.senderPublicKey, EarthsBtc) shouldBe expected
    }
  }

  property("Buy EARTHS order - filled with 2 steps, sell order - partial") {
    val counter    = buy(EarthsBtc, 100000000, 0.0008, matcherFee = Some(300001L))
    val submitted1 = sell(EarthsBtc, 50000000, 0.00075, matcherFee = Some(300001L))
    val submitted2 = sell(EarthsBtc, 80000000, 0.0008, matcherFee = Some(300001L))
    ...
    openVolume(submitted1.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L
    openVolume(submitted1.senderPublicKey, EarthsBtc.amountAsset) shouldBe 0L
    ...
    withClue(s"account checks, ord3.senderPublicKey: ${submitted2.senderPublicKey}, ord3.order.id=${submitted2.idStr()}") {
      openVolume(submitted2.senderPublicKey, EarthsBtc.priceAsset) shouldBe 0L
  ...
  property("Sell ETH twice (filled, partial), buy EARTHS order - filled") {
  ...
  property("Partially with own order") {
    val pk        = PrivateKeyAccount("private".getBytes("utf-8"))
    val counter   = buy(EarthsBtc, 100000000, 0.0008, Some(pk), Some(300000L))
    val submitted = sell(EarthsBtc, 210000000, 0.00079, Some(pk), Some(300000L))
    ...
    openVolume(pk, EarthsBtc.amountAsset) shouldBe 110157143L
    openVolume(pk, EarthsBtc.priceAsset) shouldBe 0L

    withClue("orders list") {
      activeOrderIds(pk) shouldBe Seq(submitted.id())
      allOrderIds(pk) shouldBe Seq(submitted.id(), counter.id())

      activeOrderIdsByPair(pk, EarthsBtc) shouldBe Seq(submitted.id())
      allOrderIdsByPair(pk, EarthsBtc) shouldBe Seq(submitted.id(), counter.id())
    }
  }
  ...
  property("Cancel partially executed order") {
    val counter   = sell(EarthsBtc, 2100000000, 0.0008, matcherFee = Some(300000L))
    val submitted = buy(EarthsBtc, 1000000000, 0.00081, matcherFee = Some(300000L))
    ...
    withClue("orders list of counter owner") {
      ...
      activeOrderIdsByPair(addr, EarthsBtc) shouldBe empty
      allOrderIdsByPair(addr, EarthsBtc) shouldBe Seq(counter.id())
    }
  ...
  property("Sorting by status then timestamp") {
    val pk   = PrivateKeyAccount("private".getBytes("utf-8"))
    val ord1 = buy(EarthsBtc, 110000000, 0.0008, Some(pk), Some(300000L), Some(1L)) // Filled
    val ord2 = buy(EarthsBtc, 120000000, 0.0006, Some(pk), Some(300000L), Some(2L)) // Accepted
    val ord3 = buy(EarthsBtc, 130000000, 0.0005, Some(pk), Some(300000L), Some(3L)) // Canceled
    val ord4 = sell(EarthsBtc, 2100000000, 0.00079, Some(pk), Some(300000L), Some(4L)) // Partial
    val ord5 = buy(EarthsBtc, 130000000, 0.0004, Some(pk), Some(300000L), Some(45)) // Accepted
    ...
    withClue("orders list") {
      ...
      activeOrderIdsByPair(addr, EarthsBtc) shouldBe activeOrders
      allOrderIdsByPair(addr, EarthsBtc) shouldBe allOrders
    }
  }
  ...
  property("History with more than max limit") {
    ...
    val origOrders = (0 until matcherSettings.maxOrdersPerRequest).map { i =>
      val o = buy(EarthsBtc, 100000000, 0.0008 + 0.00001 * i, Some(pk), Some(300000L), Some(100L + i))
    ...
    val newOrder = buy(EarthsBtc, 100000000, 0.001, Some(pk), Some(300000L), Some(1L))
  ...
  property("History by pair - added orders more than history by pair limit (200 active)") {
    ...
    withClue("pair") {
      ...
      allOrderIdsByPair(pk, EarthsBtc) shouldBe expectedIds
      activeOrderIdsByPair(pk, EarthsBtc) shouldBe expectedIds
    }
  }
  ...
  property("History by pair - added and canceled orders both more than history by pair limit (200 active, 10 canceled)") {
    ...
    withClue("pair") {
      val expectedIds = allOrders.takeRight(MaxElements).map(_.id()).reverse
      val pair1Ids    = allOrderIdsByPair(pk, EarthsBtc)
      pair1Ids shouldBe expectedIds
      activeOrderIdsByPair(pk, EarthsBtc) shouldBe expectedActiveOrderIds
    }
  }
