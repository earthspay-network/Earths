...
import com.earthspay.matcher.api.{OperationTimedOut, OrderAccepted, OrderCanceled}
import com.earthspay.matcher.fixtures.RestartableActor
import com.earthspay.matcher.fixtures.RestartableActor.RestartActor
import com.earthspay.matcher.market.OrderBookActor._
import com.earthspay.matcher.market.OrderHistoryActor.{ValidateOrder, ValidateOrderResult}
import com.earthspay.matcher.model.{BuyLimitOrder, LimitOrder, OrderBook, SellLimitOrder}
import com.earthspay.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings, WalletSettings}
...
class OrderBookActorSpecification extends MatcherSpec("OrderBookActor") with ImplicitSender with MatcherTestData with PathMockFactory {
  ...
  val defaultPair            = AssetPair(Some(ByteStr("BTC".getBytes)), Some(ByteStr("EARTHS".getBytes)))
    ...
    .returns(
      Portfolio(hugeAmount,
                ...
                Map(
                  ...
                  ByteStr("EARTHS".getBytes) -> hugeAmount
                )))
  ...
  "OrderBookActor" should {
    ...
    "partially execute order with zero fee remaining part" in obcTest { (pair, actor) =>
      val ord1 = sell(pair, 0.0006999, 1500 * Constants.UnitsInEarth)
    ...
    "partially execute order with price > 1 and zero fee remaining part " in obcTest { (pair, actor) =>
      ...
      val ord1 = sell(pair, 1850, (0.1 * Constants.UnitsInEarth).toLong)
      val ord2 = sell(pair, 1840, (0.01 * Constants.UnitsInEarth).toLong)
      val ord3 = buy(pair, 2000, (0.0100001 * Constants.UnitsInEarth).toLong)
    ...
    "buy small amount of pricey asset" in obcTest { (pair, actor) =>
      val p = AssetPair(Some(ByteStr("EARTHS".getBytes)), Some(ByteStr("USD".getBytes)))
