...
import com.earthspay.matcher.api.{OrderAccepted, OrderCanceled}
...
import com.earthspay.state.{ByteStr, Diff}
...
class OrderBookActorSpecification extends MatcherSpec("OrderBookActor") with NTPTime with ImplicitSender with MatcherTestData with PathMockFactory {
  ...
  "OrderBookActor" should {
    ...
    "partially execute order with zero fee remaining part" in obcTest { (pair, actor) =>
      val ord1 = sell(pair, 1500 * Constants.UnitsInEarth, 0.0006999)
    ...
    "partially execute order with price > 1 and zero fee remaining part " in obcTest { (_, actor) =>
      ...
      val ord1 = sell(pair, (0.1 * Constants.UnitsInEarth).toLong, 1850)
      val ord2 = sell(pair, (0.01 * Constants.UnitsInEarth).toLong, 1840)
      val ord3 = buy(pair, (0.0100001 * Constants.UnitsInEarth).toLong, 2000)
