...
import com.earthspay.state.{AssetDescription, Blockchain, LeaseBalance, Portfolio}
...
import com.earthspay.transaction.{AssetId, Proofs}
...
class OrderValidatorSpecification
  ...
  "OrderValidator" should {
    "allow buying EARTHS for BTC without balance for order fee" in asa() { v =>
      v shouldBe 'right
    }
  ...
  private def mkOrderValidator(bc: Blockchain, tc: ExchangeTransactionCreator) =
    OrderValidator.blockchainAware(bc, tc.createTransaction, (0.003 * Constants.UnitsInEarth).toLong, MatcherAccount, ntpTime)(_)
