...
import com.earthspay.utils.randomBytes
import com.earthspay.{NoShrink, TestTime, WithDB}
...
class OrderValidatorSpecification
  ...
  private val pairEarthsBtc = AssetPair(None, Some(wbtc))
  private val defaultTs    = 1000

  private val defaultPortfolio = Portfolio(0, LeaseBalance.empty, Map(wbtc -> 10 * Constants.UnitsInEarth))

  "OrderValidator" should {
    "allow buying EARTHS for BTC without balance for order fee" in
    ...
    "reject new order" when {
      "asset balance is negative" in portfolioTest(Portfolio(0, LeaseBalance.empty, Map(wbtc -> -10 * Constants.UnitsInEarth))) { (ov, _) =>
      ...
      "order price has invalid non-zero trailing decimals" in forAll(assetIdGen(1), accountGen, Gen.choose(1, 7)) {
        case (Some(amountAsset), sender, amountDecimals) =>
          portfolioTest(Portfolio(11 * Constants.UnitsInEarth, LeaseBalance.empty, Map.empty)) { (ov, bc) =>
            ...
            ov.validateNewOrder(buy(AssetPair(Some(amountAsset), None),
                                    10 * Constants.UnitsInEarth,
                                    price,
                                    matcherFee = Some((0.003 * Constants.UnitsInEarth).toLong))) should produce("Invalid price")
    ...
    "validate order with smart token" when {
      ...
      val portfolio = Portfolio(10 * Constants.UnitsInEarth,
                                LeaseBalance.empty,
                                Map(
                                  asset1 -> 10 * Constants.UnitsInEarth,
                                  asset2 -> 10 * Constants.UnitsInEarth
                                ))
  ...
  private def newBuyOrder: Order =
    buy(pair = pairEarthsBtc, amount = 100 * Constants.UnitsInEarth, price = 0.0022, matcherFee = Some((0.003 * Constants.UnitsInEarth).toLong))

  private def newBuyOrder(ts: Long): Order =
    buy(pair = pairEarthsBtc,
        amount = 100 * Constants.UnitsInEarth,
        price = 0.0022,
        matcherFee = Some((0.003 * Constants.UnitsInEarth).toLong),
        ts = Some(ts))
