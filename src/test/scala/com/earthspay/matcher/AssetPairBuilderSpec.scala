...
import com.earthspay.state.{AssetDescription, Blockchain, ByteStr}
...
class AssetPairBuilderSpec extends FreeSpec with Matchers with MockFactory {
  ...
  private val priceAssets = ConfigFactory.parseString(s"""earths.matcher {
  ...
  private val pairs = Table(
    ("amount", "price", "result"),
    (EARTHS, WUSD.base58, Right(())),
    (WUSD.base58, EARTHS, Left("Pair should be reverse")),
    (WBTC.base58, WEUR.base58, Left("Pair should be reverse")),
    (WEUR.base58, WBTC.base58, Right(())),
    (Asset1.base58, EARTHS, Right(())),
    (EARTHS, Asset1.base58, Left("Pair should be reverse")),
