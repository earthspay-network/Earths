...
import com.earthspay.TestHelpers.deleteRecursively
...
class MatcherSnapshotStoreSpec extends SnapshotStoreSpec(loadConfig(parseString(s"""$DirKey = ${createTempDirectory("matcher").toAbsolutePath}
         ...
         |  persistence.snapshot-store.plugin = earths.matcher.snapshot-store
         |}""".stripMargin))) {
...
object MatcherSnapshotStoreSpec {
  val DirKey = "earths.matcher.snapshot-store.dir"
}
