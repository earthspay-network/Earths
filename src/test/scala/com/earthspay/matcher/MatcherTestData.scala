...
import com.earthspay.matcher.model.{BuyLimitOrder, SellLimitOrder}
import com.earthspay.matcher.queue.{QueueEvent, QueueEventWithMeta}
import com.earthspay.settings.loadConfig
import com.earthspay.transaction.assets.exchange.{AssetPair, Order, OrderType}
import com.earthspay.{NTPTime, crypto}
...
trait MatcherTestData extends NTPTime { _: Suite =>
  ...
  val earthsAssetGen: Gen[Option[Array[Byte]]] = Gen.const(None)
  ...
  val maxEarthsAmountGen: Gen[Long] = Gen.choose(1, 100000000L * 100000000L)
  ...
  def buyGenerator(pair: AssetPair,
    ...
    for {
      ...
      matcherFee: Long          <- matcherFee.map(Gen.const).getOrElse(maxEarthsAmountGen)
  ...
  def orderGenerator(sender: PrivateKeyAccount, pair: AssetPair): Gen[Order] =
    for {
      ...
      amount: Long       <- maxEarthsAmountGen
      price: Long        <- maxEarthsAmountGen
      timestamp: Long    <- createdTimeGen
      expiration: Long   <- maxTimeGen
      matcherFee: Long   <- maxEarthsAmountGen
