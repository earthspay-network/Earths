...
import com.earthspay.history._
import com.earthspay.state.diffs._
import com.earthspay.state.{Diff, EitherExt2, NgState}
...
class NgStateTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink {
  ...
  def preconditionsAndPayments(amt: Int): Gen[(GenesisTransaction, Seq[TransferTransactionV1])] =
    for {
      ...
      payments: Seq[TransferTransactionV1] <- Gen.listOfN(amt, earthsTransferGeneratorP(master, recipient))
