...
abstract class GenericTransactionSpecification[T <: com.earthspay.transaction.Transaction]
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {

  def transactionParser: com.earthspay.transaction.TransactionParserFor[T]
  def updateProofs(tx: T, p: Proofs): T
  def generator: Gen[((Seq[com.earthspay.transaction.Transaction], T))]
