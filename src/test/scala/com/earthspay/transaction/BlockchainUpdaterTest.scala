...
import com.earthspay.features.BlockchainFeatureStatus
import com.earthspay.features.FeatureProvider._
import com.earthspay.history
...
class BlockchainUpdaterTest extends FreeSpec with Matchers with HistoryTest with ShouldVerb with WithState {
  ...
  private val EarthsSettings = history.DefaultEarthsSettings.copy(
    blockchainSettings = history.DefaultEarthsSettings.blockchainSettings.copy(
      functionalitySettings = history.DefaultEarthsSettings.blockchainSettings.functionalitySettings.copy(
        featureCheckBlocksPeriod = ApprovalPeriod,
        blocksForFeatureActivation = (ApprovalPeriod * 0.9).toInt,
        preActivatedFeatures = Map.empty
      )
    ),
    featuresSettings = history.DefaultEarthsSettings.featuresSettings.copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val EarthsSettingsWithDoubling = EarthsSettings.copy(
    blockchainSettings = EarthsSettings.blockchainSettings.copy(
      functionalitySettings = EarthsSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(EarthsSettings) { domain =>
  ...
  "features rollback with block rollback" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "feature activation height is not overriden with further periods" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "feature activated only by 90% of blocks" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "features votes resets when voting window changes" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "sunny day test when known feature activated" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
    b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(EarthsSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(EarthsSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(EarthsSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
