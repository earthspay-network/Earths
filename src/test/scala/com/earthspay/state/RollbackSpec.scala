...
import com.earthspay.features.BlockchainFeatures._
import com.earthspay.features._
import com.earthspay.lagonaki.mocks.TestBlock
import com.earthspay.lang.v1.compiler.Terms.TRUE
import com.earthspay.settings.{TestFunctionalitySettings, EarthsSettings}
import com.earthspay.state.reader.LeaseDetails
import com.earthspay.transaction.ValidationError.AliasDoesNotExist
import com.earthspay.transaction.assets.{IssueTransactionV1, ReissueTransactionV1}
import com.earthspay.transaction.lease.{LeaseCancelTransactionV1, LeaseTransactionV1}
...
import com.earthspay.transaction.{CreateAliasTransactionV1, DataTransaction, GenesisTransaction, Transaction}
import com.earthspay.{NoShrink, TestTime, TransactionGen, history}
...
class RollbackSpec extends FreeSpec with Matchers with WithDomain with TransactionGen with PropertyChecks with NoShrink {
  ...
  "Rollback resets" - {
    ...
    "forget rollbacked transaction for querying" in forAll(accountGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(genesisBlock(nextTs, sender, com.earthspay.state.diffs.ENOUGH_AMT))
    ...
    "earths balances" in forAll(accountGen, positiveLongGen, accountGen, Gen.nonEmptyListOf(Gen.choose(1, 10))) {
    ...
    def createSettings(preActivatedFeatures: (BlockchainFeature, Int)*): EarthsSettings = {
      ...
      history.DefaultEarthsSettings.copy(blockchainSettings = history.DefaultEarthsSettings.blockchainSettings.copy(functionalitySettings = tfs))
    }
    ...
    "relean rollbacked transaction" in forAll(accountGen, accountGen, Gen.listOfN(66, Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(genesisBlock(ts, sender, com.earthspay.state.diffs.ENOUGH_AMT))
