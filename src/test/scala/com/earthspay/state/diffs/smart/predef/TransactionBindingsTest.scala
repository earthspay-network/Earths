...
import com.earthspay.common.state.diffs.ProduceError._
    ...
    val ctx =
      PureContext
        ...
        CryptoContext
          .build(Global) |+|
        EarthsContext
          .build(V2, new EarthsEnvironment(chainId, Coeval(null), null, EmptyBlockchain), isTokenContext = true)
    ...
    val ctx =
      PureContext.build(V2) |+|
        CryptoContext
          .build(Global) |+|
        EarthsContext
          .build(V2, new EarthsEnvironment(chainId, Coeval(t), null, EmptyBlockchain), isTokenContext = false)
