package com.earthspay.state.diffs.smart
...
package object predef {
  ...
  def scriptWithAllFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s"""${dropLastLine(scriptWithPureFunctions(tx, t))}
       |${dropLastLine(scriptWithEarthsFunctions(tx, t))}
       |${dropLastLine(scriptWithCryptoFunctions)}
       |if rnd then pure && earths else crypto""".stripMargin
  ...
  def scriptWithEarthsFunctions(tx: DataTransaction, t: TransferTransaction): String =
    s""" # Earths context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       | }
       |
       | let balances = assetBalance(tx.sender, unit) > 0 && earthsBalance(tx.sender) != 0
       |
       | let earths = txById && entries && balances && aFromPK && aFromStrOrRecip && height > 0
       | earths""".stripMargin
