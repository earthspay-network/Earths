...
class SponsorshipDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen {
  ...
  property("cannot cancel sponsorship") {
    ...
    val setup = for {
      ...
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, None, 1 * Constants.UnitsInEarth, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, None, 1 * Constants.UnitsInEarth - 1, ts + 1)
        .right
        .get
  ...
  property("cannot сhange sponsorship fee") {
    ...
    val setup = for {
      ...
      minFee <- smallFeeGen
      senderNotIssuer = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, Some(minFee), 1 * Constants.UnitsInEarth, ts + 1)
        .right
        .get
      insufficientFee = SponsorFeeTransaction
        .selfSigned(1, notSponsor, assetId, Some(minFee), 1 * Constants.UnitsInEarth - 1, ts + 1)
        .right
        .get
    } yield (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee)
