...
class AssetTransactionsDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with NoShrink with WithDB {
  ...
  property("Issue+Reissue+Burn do not break earths invariant and updates state") {
  ...
  property("Can burn non-owned alias if feature 'BurnAnyTokens' activated") {
    val setup = for {
      ...
      earthsTransfer <- earthsTransferGeneratorP(issuer, burner)
      burn = BurnTransactionV1.selfSigned(burner, issue.assetId(), assetTransfer.amount, earthsTransfer.amount, timestamp).explicitGet()
    } yield (genesis, issue, assetTransfer, earthsTransfer, burn)
    ...
    forAll(setup) {
      case (genesis, issue, assetTransfer, earthsTransfer, burn) =>
        assertDiffAndState(Seq(TestBlock.create(Seq(genesis, issue, assetTransfer, earthsTransfer))), TestBlock.create(Seq(burn)), fs) {
          case (_, newState) =>
  ...
  def genesisIssueTransferReissue(code: String): Gen[(Seq[GenesisTransaction], IssueTransactionV2, TransferTransactionV1, ReissueTransactionV1)] =
    for {
      ...
      timestamp          <- timestampGen
      initialEarthsAmount <- Gen.choose(Long.MaxValue / 1000, Long.MaxValue / 100)
      accountA           <- accountGen
      accountB           <- accountGen
      smallFee           <- Gen.choose(1l, 10l)
      genesisTx1 = GenesisTransaction.create(accountA, initialEarthsAmount, timestamp).explicitGet()
      genesisTx2 = GenesisTransaction.create(accountB, initialEarthsAmount, timestamp).explicitGet()
      reissuable = true
