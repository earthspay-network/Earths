...
import com.earthspay.lang.v1.compiler.{CompilerContext, CompilerV1}
import com.earthspay.settings.{Constants, FunctionalitySettings, TestFunctionalitySettings}
import com.earthspay.state._
import com.earthspay.state.diffs.TransactionDiffer.TransactionValidationError
...
import com.earthspay.transaction.assets.{IssueTransaction, IssueTransactionV1, IssueTransactionV2}
...
import com.earthspay.transaction.transfer.TransferTransaction
import com.earthspay.utils.functionCosts
...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("preserves earths invariant, stores match info, rewards matcher") {
  ...
  property("buy earths without enough money for fee") {
    val preconditions: Gen[(GenesisTransaction, GenesisTransaction, IssueTransactionV1, ExchangeTransaction)] = for {
      buyer  <- accountGen
      seller <- accountGen
      ts     <- timestampGen
      gen1: GenesisTransaction = GenesisTransaction.create(buyer, 1 * Constants.UnitsInEarth, ts).explicitGet()
      gen2: GenesisTransaction = GenesisTransaction.create(seller, ENOUGH_AMT, ts).explicitGet()
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    forAll(for {
      ...
      tr1     = createEarthsTransfer(MATCHER, buyer.toAddress, Long.MaxValue / 3, enoughFee, ts + 1).explicitGet()
      tr2     = createEarthsTransfer(MATCHER, seller.toAddress, Long.MaxValue / 3, enoughFee, ts + 2).explicitGet()
