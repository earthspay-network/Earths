...
import com.earthspay.mining._
import com.earthspay.settings._
import com.earthspay.state.diffs._
import com.earthspay.state.{ByteStr, EitherExt2, _}
import com.earthspay.transaction.ValidationError.SenderIsBlacklisted
...
import com.earthspay.transaction.{FeeCalculator, GenesisTransaction, Transaction}
import com.earthspay.utils.Time
import com.earthspay._
...
class UtxPoolSpecification extends FreeSpec with Matchers with MockFactory with PropertyChecks with TransactionGen with NoShrink with WithDB {
  ...
  private val calculatorSettings = FeesSettings(
    ...
    ).map(_.typeId.toInt -> List(FeeSettings("EARTHS", 0))).toMap
  )
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val origSettings    = EarthsSettings.fromConfig(config)
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs        = for (_ <- 1 to n) yield createEarthsTransfer(sender, recipient, amountPart, fee, time.getTimestamp()).explicitGet()
  ...
  private def transactionV1Gen(sender: PrivateKeyAccount, ts: Long, feeAmount: Long): Gen[TransferTransactionV1] = accountGen.map { recipient =>
    TransferTransactionV1.selfSigned(None, sender, recipient, earths(1), ts, None, feeAmount, Array.emptyByteArray).explicitGet()
  }
  ...
  private def transactionV2Gen(sender: PrivateKeyAccount, ts: Long, feeAmount: Long): Gen[TransferTransactionV2] = accountGen.map { recipient =>
    TransferTransactionV2.selfSigned(2, None, sender, recipient, earths(1), ts, None, feeAmount, Array.emptyByteArray).explicitGet()
  }
