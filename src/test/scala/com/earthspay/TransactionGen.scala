...
trait TransactionGenBase extends ScriptGen with NTPTime { _: Suite =>
  ...
  val setAssetScriptTransactionGen: Gen[(Seq[Transaction], SetAssetScriptTransaction)] = for {
    ...
    (Seq(issue),
     SetAssetScriptTransaction
       .create(version, AddressScheme.current.chainId, sender, issue.id(), script, 1 * Constants.UnitsInEarth + ScriptExtraFee, timestamp, proofs)
       .explicitGet())
  ...
  def sponsorFeeCancelSponsorFeeGen(
    ...
    for {
      ...
      (issue,
       SponsorFeeTransaction.selfSigned(1, sender, assetId, Some(minFee), 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(1, sender, assetId, Some(minFee1), 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(1, sender, assetId, None, 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
      )
