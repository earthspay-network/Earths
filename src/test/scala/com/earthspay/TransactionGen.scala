...
import com.earthspay.account.PublicKeyAccount._
...
import com.earthspay.transaction.transfer.MassTransferTransaction.{MaxTransferCount, ParsedTransfer}
...
trait TransactionGenBase extends ScriptGen with NTPTime { _: Suite =>
  ...
  protected def earths(n: Float): Long = (n * 100000000L).toLong
  ...
  val earthsAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]]    = Gen.frequency((1, earthsAssetGen), (10, Gen.option(bytes32gen.map(ByteStr(_)))))
  ...
  val setAssetScriptTransactionGen: Gen[(Seq[Transaction], SetAssetScriptTransaction)] = for {
    ...
    (Seq(issue),
     SetAssetScriptTransaction
       .create(AddressScheme.current.chainId, sender, issue.id(), script, 1 * Constants.UnitsInEarth + ScriptExtraFee, timestamp, proofs)
       .explicitGet())
  ...
  def earthsTransferGeneratorP(sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
    transferGeneratorP(sender, recipient, None, None)

  def earthsTransferGeneratorP(timestamp: Long, sender: PrivateKeyAccount, recipient: AddressOrAlias): Gen[TransferTransactionV1] =
  ...
  def createEarthsTransfer(sender: PrivateKeyAccount,
                          recipient: Address,
                          amount: Long,
                          fee: Long,
  ...
  val transferWithEarthsFeeGen = for {
  ...
  val selfTransferWithEarthsFeeGen: Gen[TransferTransactionV1] = for {
  ...
  def sponsorFeeCancelSponsorFeeGen(
    ...
    for {
      ...
      (issue,
       SponsorFeeTransaction.selfSigned(sender, assetId, Some(minFee), 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(sender, assetId, Some(minFee1), 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
       SponsorFeeTransaction.selfSigned(sender, assetId, None, 1 * Constants.UnitsInEarth, timestamp).explicitGet(),
      )
