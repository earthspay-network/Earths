...
import com.earthspay.account.{Address, AddressOrAlias, PrivateKeyAccount}
...
class BlockchainUpdaterMicroblockSunnyDayTest
  ...
  val preconditionsAndPayments: Gen[Setup] = for {
    ...
    masterToAlice: TransferTransactionV1 <- earthsTransferGeneratorP(master, alice)
    aliceToBob  = createEarthsTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts).explicitGet()
    aliceToBob2 = createEarthsTransfer(alice, bob, masterToAlice.amount - fee - 1, fee, ts + 1).explicitGet()
  ...
  property("discarding some of microBlocks doesn't affect resulting state") {
    forAll(preconditionsAndPayments, accountGen) {
      case ((genesis, masterToAlice, aliceToBob, aliceToBob2), miner) =>
        ...
        val minerABalance = withDomain(MicroblocksActivatedAt0EarthsSettings) { da =>
        ...
        val minerBBalance = withDomain(MicroblocksActivatedAt0EarthsSettings) { db =>
