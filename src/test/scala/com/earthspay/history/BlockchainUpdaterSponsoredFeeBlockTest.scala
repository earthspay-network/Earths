...
import com.earthspay.transaction.assets.{IssueTransaction, SponsorFeeTransaction}
...
class BlockchainUpdaterSponsoredFeeBlockTest
  ...
  val sponsorPreconditions: Gen[Setup] = for {
    ...
    earthsFee                    = Sponsorship.toEarths(sponsorTx.minSponsoredAssetFee.get, sponsorTx.minSponsoredAssetFee.get)
    genesis: GenesisTransaction = GenesisTransaction.create(master, ENOUGH_AMT, ts).explicitGet()
    masterToAlice: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(None,
                  master,
                  alice,
                  feeAsset.fee + sponsorTx.fee + transferAssetEarthsFee + earthsFee,
                  ts + 1,
                  None,
                  transferAssetEarthsFee,
  ...
  val SponsoredActivatedAt0EarthsSettings: EarthsSettings = settings.copy(blockchainSettings = SponsoredFeeActivatedAt0BlockchainSettings)

  property("not enough earths to sponsor sponsored tx") {
    scenario(sponsorPreconditions, SponsoredActivatedAt0EarthsSettings) {
      case (domain, (genesis, masterToAlice, feeAsset, sponsor, aliceToBob, bobToMaster, bobToMaster2)) =>
        ...
        domain.blockchainUpdater.processBlock(block4) should produce("negative earths balance" /*"unavailable funds"*/ )
