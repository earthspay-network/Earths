...
import cats.Show
import com.earthspay.account.{AddressOrAlias, PrivateKeyAccount, PublicKeyAccount}
...
import com.earthspay.dexgen.Worker._
import com.earthspay.dexgen.utils.{ApiRequests, GenOrderType}
import com.earthspay.it.api.{MatcherResponse, MatcherStatusResponse, OrderbookHistory, Transaction, UnexpectedStatusCodeException}
...
import com.earthspay.utils.LoggerFacade
...
class Worker(workerSettings: Settings,
  ...
  def transfer(sender: PrivateKeyAccount, assetId: Option[AssetId], recipient: PrivateKeyAccount, halfBalance: Boolean)(
    ...
    to(endpoint).balance(sender.address, assetId).flatMap { balance =>
      ...
      val transferAmount = assetId.fold(halfAmount - 0.001.earths)(_ => halfAmount)
      ...
      TransferTransactionV1.selfSigned(assetId,
        ...
        case Right(txRequest) =>
          log.info(
            s"[$tag] ${assetId.fold("Earths")(_.base58)} balance of ${sender.address}: $balance, sending $transferAmount to ${recipient.address}")
