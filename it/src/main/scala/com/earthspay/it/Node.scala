...
import com.earthspay.utils.{Base58, LoggerFacade}
...
abstract class Node(config: Config) extends AutoCloseable {
  ...
  val settings: EarthsSettings = EarthsSettings.fromConfig(config)
...
object Node {
  implicit class NodeExt(val n: Node) extends AnyVal {
    ...
    def fee(txTypeId: Byte, asset: String = "EARTHS"): Long = n.settings.feesSettings.fees(txTypeId).find(_.asset == asset).get.fee
