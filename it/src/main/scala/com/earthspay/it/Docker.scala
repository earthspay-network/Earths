...
import com.earthspay.it.util.GlobalTimer.{instance => timer}
...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false) extends AutoCloseable with ScorexLogging {
  ...
  private val logDir: Coeval[Path] = Coeval.evalOnce {
    val r = Option(System.getProperty("earths.it.logging.dir"))
      .map(Paths.get(_))
  ...
  private lazy val earthsNetwork: Network = {
    val networkName = s"earths-${hashCode().toLong.toHexString}"
  ...
  private def startNodeInternal(nodeConfig: Config): DockerNode =
    try {
      ...
      val restApiPort    = actualConfig.getString("earths.rest-api.port")
      val networkPort    = actualConfig.getString("earths.network.port")
      val matcherApiPort = actualConfig.getString("earths.matcher.port")
      ...
      val nodeName   = actualConfig.getString("earths.network.node-name")
      ...
      val configOverrides: String = {
        val ntpServer    = Option(System.getenv("NTP_SERVER")).fold("")(x => s"-Dearths.ntp-server=$x ")
        val maxCacheSize = Option(System.getenv("MAX_CACHE_SIZE")).fold("")(x => s"-Dearths.max-cache-size=$x ")
        val kafkaServer = Option(System.getenv("KAFKA_SERVER")).fold("") { x =>
          val prefix = "-Dearths.matcher.events-queue"
        ...
        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dearths.network.declared-address=$ip:$networkPort $ntpServer $maxCacheSize $kafkaServer"
        ...
        if (enableProfiling) {
          ...
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2019.1/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=EarthsNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot,onexit=snapshot "
        }
      ...
      val containerConfig = ContainerConfig
        ...
        .networkingConfig(ContainerConfig.NetworkingConfig.create(Map(
          earthsNetwork.name() -> endpointConfigFor(nodeName)
        ).asJava))
        .hostConfig(hostConfig)
        .env(s"EARTHS_OPTS=$configOverrides")
        .build()

      val containerId = {
        val containerName = s"${earthsNetwork.name()}-$nodeName"
        dumpContainers(
      ...
      val node = new DockerNode(actualConfig, containerId, getNodeInfo(containerId, EarthsSettings.fromConfig(actualConfig)))
      nodes.add(node)
  ...
  private def getNodeInfo(containerId: String, settings: EarthsSettings): NodeInfo = {
    ...
    val earthsIpAddress = containerInfo.networkSettings().networks().get(earthsNetwork.name()).ipAddress()
    ...
    NodeInfo(
      ...
      new InetSocketAddress(earthsIpAddress, networkPort)
    )
  }

  private def inspectContainer(containerId: String): ContainerInfo = {
    val containerInfo = client.inspectContainer(containerId)
    if (containerInfo.networkSettings().networks().asScala.contains(earthsNetwork.name())) containerInfo
    else {
      log.debug(s"Container $containerId has not connected to the network ${earthsNetwork.name()} yet, retry")
      Thread.sleep(1000)
  ...
  def restartNode(node: DockerNode, configUpdates: Config = empty): DockerNode = {
    ...
    if (configUpdates != empty) {
      ...
      val shPath = "/opt/earths/start-earths.sh"
      val scriptCmd: Array[String] =
        Array("sh", "-c", s"sed -i 's|$$EARTHS_OPTS.*-jar|$$EARTHS_OPTS $renderedConfig -jar|' $shPath && chmod +x $shPath")
  ...
  override def close(): Unit = {
    if (isStopped.compareAndSet(false, true)) {
      ...
      try {
        client.removeNetwork(earthsNetwork.id)
      } catch {
        case NonFatal(e) =>
          // https://github.com/moby/moby/issues/17217
          log.warn(s"Can not remove network ${earthsNetwork.name()}", e)
      }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = {
    ...
    client.disconnectFromNetwork(containerId, earthsNetwork.id())
  }
  ...
  private def updateStartScript(node: DockerNode): Unit = {
    ...
    log.debug("Make backup copy of /opt/earths/start-earths.sh")
    val cpCmd: Array[String] =
      Array(
        "sh",
        "-c",
        s"""cp /opt/earths/start-earths.sh /opt/earths/start-earths.sh.bk"""
      )
    ...
    val scriptCmd: Array[String] =
      Array(
        "sh",
        "-c",
        s"""rm /opt/earths/start-earths.sh && echo '#!/bin/bash' >> /opt/earths/start-earths.sh &&
             |echo 'java ${renderProperties(asProperties(genesisOverride))} -cp /opt/earths/earths.jar com.earthspay.matcher.MatcherTool /opt/earths/template.conf cb > /opt/earths/migration-tool.log' >> /opt/earths/start-earths.sh &&
             |echo 'less /opt/earths/migration-tool.log | grep -ir completed && cp /opt/earths/start-earths.sh.bk /opt/earths/start-earths.sh && chmod +x /opt/earths/start-earths.sh' >> /opt/earths/start-earths.sh &&
             |chmod +x /opt/earths/start-earths.sh
           """.stripMargin
      )
...
object Docker {
  private val ContainerRoot = Paths.get("/opt/earths")
  ...
  def genesisOverride = {
    ...
    val timestampOverrides = parseString(s"""earths.blockchain.custom.genesis {
                                            |  timestamp = $genesisTs
                                            |  block-timestamp = $genesisTs
                                            |}""".stripMargin)

    val genesisConfig    = configTemplate.withFallback(timestampOverrides)
    val gs               = genesisConfig.as[GenesisSettings]("earths.blockchain.custom.genesis")
    val genesisSignature = Block.genesis(gs).explicitGet().uniqueId

    timestampOverrides.withFallback(parseString(s"earths.blockchain.custom.genesis.signature = $genesisSignature"))
  }

  AddressScheme.current = new AddressScheme {
    override val chainId = configTemplate.as[String]("earths.blockchain.custom.address-scheme-character").charAt(0).toByte
  }
