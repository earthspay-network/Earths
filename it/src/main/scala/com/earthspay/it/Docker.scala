...
class Docker(suiteConfig: Config = empty, tag: String = "", enableProfiling: Boolean = false) extends AutoCloseable with ScorexLogging {
    ...
    try {
      ...
      val configOverrides: String = {
        ...
        var config = s"$javaOptions ${renderProperties(asProperties(overrides))} " +
          s"-Dlogback.stdout.level=TRACE -Dlogback.file.level=OFF -Dearths.network.declared-address=$ip:$networkPort $ntpServer"

        if (enableProfiling) {
          config += s"-agentpath:/usr/local/YourKit-JavaProfiler-2018.04/bin/linux-x86-64/libyjpagent.so=port=$ProfilerPort,listen=all," +
            s"sampling,monitors,sessionname=EarthsNode,dir=$ContainerRoot/profiler,logdir=$ContainerRoot "
        }
  ...
  private def disconnectFromNetwork(containerId: String): Unit = client.disconnectFromNetwork(containerId, earthsNetwork.id())
