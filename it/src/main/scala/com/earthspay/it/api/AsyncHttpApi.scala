...
import com.earthspay.api.http.alias.CreateAliasV1Request
import com.earthspay.api.http.assets._
import com.earthspay.api.http.leasing.{LeaseCancelV1Request, LeaseV1Request, SignedLeaseCancelV1Request, SignedLeaseV1Request}
import com.earthspay.api.http.{AddressApiRoute, ConnectReq, DataRequest}
import com.earthspay.features.api.ActivationStatus
import com.earthspay.http.DebugApiRoute._
import com.earthspay.http.DebugMessage._
import com.earthspay.http.{DebugMessage, RollbackParams, api_key}
import com.earthspay.it.Node
import com.earthspay.it.util.GlobalTimer.{instance => timer}
import com.earthspay.it.util._
import com.earthspay.state.{DataEntry, EitherExt2, Portfolio}
...
object AsyncHttpApi extends Assertions {
  ...
  implicit class NodeAsyncHttpApi(n: Node) extends Assertions with Matchers {
    ...
    def payment(sourceAddress: String, recipient: String, amount: Long, fee: Long): Future[Transaction] =
      postJson("/earths/payment", PaymentRequest(amount, fee, sourceAddress, recipient)).as[Transaction]

    def lease(sourceAddress: String, recipient: String, amount: Long, fee: Long, version: Byte = 2): Future[Transaction] = {
      version match {
        case 2 => { //TODO: @monroid refactor after https://earthspay.atlassian.net/browse/NODE-1222 fix
          signAndBroadcast(
            Json.obj(
              "type"      -> 8,
    ...
    def debugStateAt(height: Long): Future[Map[String, Long]] = getWithApiKey(s"/debug/stateEarths/$height").as[Map[String, Long]]
