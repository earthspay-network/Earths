package com.earthspay.it.async.matcher
...
import com.earthspay.it.api.AsyncHttpApi.NodesAsyncHttpApi
import com.earthspay.it.api.AsyncMatcherHttpApi._
import com.earthspay.it.async.matcher.CorrectStatusAfterPlaceTestSuite._
...
class CorrectStatusAfterPlaceTestSuite extends FreeSpec with Matchers with NodesFromDocker {
  ...
  override protected val nodeConfigs: Seq[Config] =
    List(NodeConfigs.Default.last, Random.shuffle(NodeConfigs.Default.init).head)
      ...
      .map { case (n, o) => o.withFallback(ConfigFactory.parseString("earths.miner.quorum=1")).withFallback(n) }
  ...
  "place orders and check their statuses" in {
    ...
    val orders = for {
      ...
      pair    <- pairs
      i       <- 1 to 60
    } yield matcherNode.prepareOrder(account, pair, OrderType.SELL, 100000L, 10000L, 0.003.earths, 1, timestamp = ts + i)
