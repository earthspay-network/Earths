package com.earthspay.it.sync.smartcontract.smartasset

import com.earthspay.it.NTPTime
import com.earthspay.it.api.SyncHttpApi._
import com.earthspay.it.sync._
import com.earthspay.it.sync.smartcontract.{cryptoContextScript, pureContextScript, earthsContextScript, _}
...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("AssetPair from smart assets") {
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.earths") {
      ...
      assertBadRequestAndMessage(
        ...
        "com.earthspay.transaction.assets.exchange.ExchangeTransactionV2 does not exceed minimal value of 1500000"
      )
  ...
  test("use all functions from RIDE for asset script") {
    ...
    val script3 = Some(ScriptCompiler(earthsContextScript(dtx, false), isAssetScript = true).explicitGet()._1.bytes.value.base64)
