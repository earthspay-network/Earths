...
import com.earthspay.transaction.lease.LeaseTransaction.Status.{Active, Canceled}
...
object LeaseStatusTestSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""earths {
       |   miner{
       |      enable = yes
  ...
  private val notMinerConfig = ConfigFactory.parseString(s"""earths {
       |   miner.enable = no
