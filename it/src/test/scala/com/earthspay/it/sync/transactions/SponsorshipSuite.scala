...
import com.earthspay.api.http.assets.SignedSponsorFeeRequest
...
import com.earthspay.it.{NodeConfigs, ReportingTestName}
import com.earthspay.state.{ByteStr, Sponsorship}
import com.earthspay.transaction.assets.SponsorFeeTransaction
...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  override def nodeConfigs: Seq[Config] =
    NodeConfigs.newBuilder
      ...
      .overrideBase(_.raw("earths.blockchain.custom.functionality.blocks-for-feature-activation=1"))
      .overrideBase(_.raw("earths.blockchain.custom.functionality.feature-check-blocks-period=1"))
      .withDefault(1)
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val sponsorEarthsBalance = sponsor.accountBalances(sponsor.address)._2
    val minerEarthsBalance   = miner.accountBalances(miner.address)._2
    ...
    "check balance before test accounts balances" in {
      ...
      sponsor.assertBalances(sponsor.address, sponsorEarthsBalance - 2 * issueFee - minFee)
    ...
    val minerEarthsBalanceAfterFirstXferTest   = minerEarthsBalance + 2.earths + minFee + Sponsorship.FeeUnit * SmallFee / minSponsorFee
    val sponsorEarthsBalanceAfterFirstXferTest = sponsorEarthsBalance - 2.earths - minFee - Sponsorship.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
      ...
      "sponsor should receive sponsored asset as fee, earths should be written off" in {
        miner.assertAssetBalance(sponsor.address, sponsorAssetId, sponsorAssetTotal / 2 + SmallFee)
        miner.assertBalances(sponsor.address, sponsorEarthsBalanceAfterFirstXferTest)
      }

      "miner earths balance should be changed" in {
        miner.assertBalances(miner.address, minerEarthsBalanceAfterFirstXferTest)
      }
    }

    "assets balance should contain sponsor fee info and sponsor balance" in {
      val sponsorLeaseSomeEarths = sponsor.lease(sponsor.address, bob.address, leasingAmount, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseSomeEarths)
    ...
    "earths fee depends on sponsor fee and sponsored token decimals" in {
      val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.earths, LargeFee, None, Some(sponsorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
      assert(!transferTxCustomFeeAlice.isEmpty)
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorEarthsBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee,
        sponsorEarthsBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerEarthsBalanceAfterFirstXferTest + Sponsorship.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorEarthsBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorEarthsBalanceAfterFirstXferTest - Sponsorship.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerEarthsBalanceAfterFirstXferTest + Sponsorship.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val aliceEarthsBalance   = alice.accountBalances(alice.address)
        val bobAssetBalance     = bob.assetBalance(bob.address, sponsorAssetId).balance
        val bobEarthsBalance     = bob.accountBalances(bob.address)
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.earths, TinyFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        nodes.waitForHeightArise()

        val earthsFee = Sponsorship.FeeUnit * TinyFee / TinyFee
        sponsor.assertBalances(sponsor.address, sponsoredBalance._1 - earthsFee, sponsoredBalance._2 - earthsFee)
        ...
        alice.assertBalances(alice.address, aliceEarthsBalance._2 - 1.earths)
        bob.assertBalances(bob.address, bobEarthsBalance._1 + 1.earths, bobEarthsBalance._2 + 1.earths)
        bob.assertAssetBalance(bob.address, sponsorAssetId, bobAssetBalance)
        miner.assertBalances(miner.address, minerBalance._2 + earthsFee)
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.earths, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val earthsFee = Sponsorship.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val aliceTransferEarths = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(sponsorAssetId2)).id
      nodes.waitForHeightAriseAndTxPresent(aliceTransferEarths)
      nodes.waitForHeightArise()

      val totalEarthsFee = Sponsorship.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
      miner.assertBalances(miner.address, minerBalance._1 + totalEarthsFee)
      sponsor.assertBalances(sponsor.address, sponsorBalance._1 - totalEarthsFee, sponsorBalance._2 - totalEarthsFee)
    ...
    "miner is sponsor" in {
      ...
      val aliceSponsoredTransferEarths = alice.transfer(alice.address, bob.address, transferAmount, SmallFee, None, Some(minersSpondorAssetId)).id
      nodes.waitForHeightAriseAndTxPresent(aliceSponsoredTransferEarths)
      nodes.waitForHeightArise()
    ...
    "tx is declined if sponsor has not enough effective balance to pay fee" in {
      ...
      val sponsorLeaseAllAvaliableEarths             = sponsor.lease(sponsor.address, bob.address, sponsorEffectiveBalance - leasingFee, leasingFee).id
      nodes.waitForHeightAriseAndTxPresent(sponsorLeaseAllAvaliableEarths)
      assertBadRequestAndMessage(alice.transfer(alice.address, bob.address, 10 * Token, LargeFee, Some(sponsorAssetId), Some(sponsorAssetId)),
                                 "unavailable funds")
      val cancelLeasingTx = sponsor.cancelLease(sponsor.address, sponsorLeaseAllAvaliableEarths, leasingFee).id
