...
class SponsorshipSuite extends FreeSpec with NodesFromDocker with Matchers with ReportingTestName with CancelAfterFailure {
  ...
  "Fee in sponsored asset works fine" - {
    ...
    val minerEarthsBalanceAfterFirstXferTest   = minerEarthsBalance + 2.earths + minFee + CommonValidation.FeeUnit * SmallFee / minSponsorFee
    val sponsorEarthsBalanceAfterFirstXferTest = sponsorEarthsBalance - 2.earths - minFee - CommonValidation.FeeUnit * SmallFee / minSponsorFee
...
//        sponsorTx.head.count(tx => tx.sender.contains(sponsor.address) || tx.recipient.contains(sponsor.address)) shouldBe 4
    ...
    "earths fee depends on sponsor fee and sponsored token decimals" in {
      ...
      miner.assertBalances(
        sponsor.address,
        sponsorEarthsBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee,
        sponsorEarthsBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount
      )
      miner.assertBalances(miner.address, minerEarthsBalanceAfterFirstXferTest + CommonValidation.FeeUnit * LargeFee / Token + leasingFee)
    }
    ...
    "cancel sponsorship" - {
      ...
      "check sponsor and miner balances after cancel" in {
        miner.assertBalances(
          sponsor.address,
          sponsorEarthsBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - issueFee,
          sponsorEarthsBalanceAfterFirstXferTest - CommonValidation.FeeUnit * LargeFee / Token - leasingFee - leasingAmount - issueFee
        )
        miner.assertBalances(miner.address, minerEarthsBalanceAfterFirstXferTest + CommonValidation.FeeUnit * LargeFee / Token + leasingFee + issueFee)
      }
    ...
    "set sponsopship again" - {
      ...
      "make transfer with new min sponsored fee" in {
        ...
        val earthsFee = CommonValidation.FeeUnit * TinyFee / TinyFee
      ...
      "make transfer with updated min sponsored fee" in {
        ...
        val transferTxCustomFeeAlice = alice.transfer(alice.address, bob.address, 1.earths, LargeFee, None, Some(sponsorAssetId)).id
        nodes.waitForHeightAriseAndTxPresent(transferTxCustomFeeAlice)
        val earthsFee = CommonValidation.FeeUnit * LargeFee / LargeFee
        nodes.waitForHeightArise()
    ...
    "issue asset make sponsor and burn and reissue" in {
      ...
      val totalEarthsFee = CommonValidation.FeeUnit * SmallFee / Token + issueFee + sponsorFee + burnFee + minFee + issueFee
