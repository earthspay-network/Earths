...
class TradeBalanceAndRoundingTestSuite
  ...
  "Alice and Bob trade EARTHS-USD" - {
    ...
    "place usd-earths order" in {
      ...
      val bobOrder1   = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, price, sellOrderAmount)
      ...
      val aliceOrder   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, price, buyOrderAmount)
    ...
    "check earths-usd tradable balance" in {
      ...
      matcherNode.cancelOrder(bobNode, earthsUsdPair, Some(orderId))
  ...
  "Alice and Bob trade EARTHS-USD check CELLING" - {
    ...
    "place usd-earths order" in {
      ...
      val bobOrder1   = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, price2, sellOrderAmount2)
      val bobOrder1Id = matcherNode.placeOrder(bobOrder1).message.id
      matcherNode.waitOrderStatus(earthsUsdPair, bobOrder1Id, "Accepted", 1.minute)
      ...
      val aliceOrder   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, price2, buyOrderAmount2)
      val aliceOrderId = matcherNode.placeOrder(aliceOrder).message.id
      ...
      val exchangeTx = matcherNode.transactionsByOrder(aliceOrder.id().base58).headOption.getOrElse(fail("Expected an exchange transaction"))
      nodes.waitForHeightAriseAndTxPresent(exchangeTx.id)
      matcherNode.cancelOrder(bobNode, earthsUsdPair, Some(bobOrder1Id))
    }
  ...
  "Alice and Bob trade WCT-EARTHS on not enough fee when place order" - {
    ...
    "bob lease all earths exact half matcher fee" in {
      ...
      val bobOrderId = matcherNode.placeOrder(bobNode, wctEarthsPair, SELL, wctEarthsPrice, wctEarthsSellAmount).message.id
      matcherNode.waitOrderStatus(wctEarthsPair, bobOrderId, "Accepted", 1.minute)

      matcherNode.tradableBalance(bobNode, wctEarthsPair)("EARTHS") shouldBe matcherFee / 2 + receiveAmount(SELL, wctEarthsPrice, wctEarthsSellAmount) - matcherFee
      matcherNode.cancelOrder(bobNode, wctEarthsPair, Some(bobOrderId))

      assertBadRequestAndResponse(matcherNode.placeOrder(bobNode, wctEarthsPair, SELL, wctEarthsPrice, wctEarthsSellAmount / 2),
  ...
  "Alice and Bob trade ETH-EARTHS" - {
    "reserved balance is empty after the total execution" in {
      val counterId1 = matcherNode.placeOrder(aliceNode, ethEarthsPair, SELL, 300000, 2864310).message.id
      matcherNode.waitOrderStatus(ethEarthsPair, counterId1, "Accepted", 1.minute)

      val counterId2 = matcherNode.placeOrder(aliceNode, ethEarthsPair, SELL, 300000, 7237977).message.id
      matcherNode.waitOrderStatus(ethEarthsPair, counterId2, "Accepted", 1.minute)

      val submittedId = matcherNode.placeOrder(bobNode, ethEarthsPair, BUY, 300000, 4373667).message.id
      ...
      matcherNode.reservedBalance(bobNode) shouldBe empty
      matcherNode.cancelOrder(aliceNode, ethEarthsPair, Some(counterId2))
    }
  }

  "Submitted order Canceled during match" in {
    val bobOrder   = matcherNode.prepareOrder(matcherNode, earthsUsdPair, OrderType.SELL, 10L, 10000000L)
    ...
    val aliceOrder   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, 1000L, 100000L)
    val aliceOrderId = matcherNode.placeOrder(aliceOrder).message.id
    ...
    matcherNode.cancelOrder(matcherNode, earthsUsdPair, Some(bobOrderId))
  }
