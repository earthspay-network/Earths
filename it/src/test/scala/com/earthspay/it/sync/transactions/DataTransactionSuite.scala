...
import com.earthspay.it.api.UnexpectedStatusCodeException
import com.earthspay.it.sync.{calcDataFee, minFee}
import com.earthspay.it.transactions.BaseTransactionSuite
import com.earthspay.it.util._
import com.earthspay.state.{BinaryDataEntry, BooleanDataEntry, ByteStr, DataEntry, EitherExt2, IntegerDataEntry, StringDataEntry}
import com.earthspay.utils.Base58
import org.scalatest.{Assertion, Assertions}
import play.api.libs.json._
import com.earthspay.api.http.SignedDataRequest
import com.earthspay.transaction.DataTransaction
...
class DataTransactionSuite extends BaseTransactionSuite {
  ...
  test("sender's earths balance is decreased by fee.") {
  ...
  test("cannot transact without having enough earths") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)

    val data = List(BooleanDataEntry("bool", false))
    assertBadRequestAndResponse(sender.putData(firstAddress, data, balance1 + 1), "negative earths balance")
    nodes.waitForHeightArise()
    notMiner.assertBalances(firstAddress, balance1, eff1)

    val leaseAmount = 1.earths
