...
class RoundingIssuesTestSuite
  ...
  "should correctly fill an order with small amount" in {
    ...
    val counter   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, 238, 3100000000L)
    val counterId = matcherNode.placeOrder(counter).message.id

    val submitted   = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, 235, 425532L)
    ...
    matcherNode.cancelOrder(aliceNode, earthsUsdPair, Some(counterId))
  ...
  "should correctly fill 2 counter orders" in {
    val counter1 = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, 60L, 98333333L)
    matcherNode.placeOrder(counter1).message.id

    val counter2   = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, 70L, 100000000L)
    val counter2Id = matcherNode.placeOrder(counter2).message.id

    val submitted   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, 1000L, 100000000L)
    val submittedId = matcherNode.placeOrder(submitted).message.id
