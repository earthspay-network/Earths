...
class RoundingIssuesTestSuite extends MatcherSuiteBase {
  ...
  "should correctly fill an order with small amount" in {
    ...
    val counter   = matcherNode.prepareOrder(aliceAcc, earthsUsdPair, OrderType.BUY, 3100000000L, 238)
    val counterId = matcherNode.placeOrder(counter).message.id

    val submitted   = matcherNode.prepareOrder(bobAcc, earthsUsdPair, OrderType.SELL, 425532L, 235)
    ...
    matcherNode.cancelOrder(aliceAcc, earthsUsdPair, counterId)
  ...
  "should correctly fill 2 counter orders" in {
    val counter1 = matcherNode.prepareOrder(bobAcc, earthsUsdPair, OrderType.SELL, 98333333L, 60L)
    matcherNode.placeOrder(counter1).message.id

    val counter2   = matcherNode.prepareOrder(bobAcc, earthsUsdPair, OrderType.SELL, 100000000L, 70L)
    val counter2Id = matcherNode.placeOrder(counter2).message.id

    val submitted   = matcherNode.prepareOrder(aliceAcc, earthsUsdPair, OrderType.BUY, 100000000L, 1000L)
    val submittedId = matcherNode.placeOrder(submitted).message.id
