...
import com.earthspay.api.http.assets.{MassTransferRequest, SignedMassTransferRequest}
import com.earthspay.transaction.transfer.MassTransferTransaction.{MaxTransferCount, Transfer}
import com.earthspay.transaction.transfer.TransferTransaction.MaxAttachmentSize
...
class MassTransferTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("asset mass transfer changes asset balances and sender's.earths balance is decreased by fee.") {
  ...
  test("earths mass transfer changes earths balances") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("can not make mass transfer without having enough earths") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
    val transfers        = List(Transfer(secondAddress, balance1 / 2), Transfer(thirdAddress, balance1 / 2))

    assertBadRequestAndResponse(sender.massTransfer(firstAddress, transfers, calcMassTransferFee(transfers.size)), "negative earths balance")
  ...
  test("reporting MassTransfer transactions") {
    ...
    val transfers = List(Transfer(firstAddress, 5.earths), Transfer(secondAddress, 2.earths), Transfer(thirdAddress, 3.earths))
    ...
    assert((txSender \ "totalAmount").as[Long] == 10.earths)
    ...
    assert((txRecipient \ "totalAmount").as[Long] == 10.earths)
  ...
  test("reporting MassTransfer transactions to aliases") {
    ...
    val transfers = aliases.map { alias =>
      Transfer(Alias.buildWithCurrentNetworkByte(alias).explicitGet().stringRepr, 2.earths)
    }
    val txId = sender.massTransfer(firstAddress, transfers, 300000).id
    nodes.waitForHeightAriseAndTxPresent(txId)
