...
import com.earthspay.transaction.assets.exchange.{AssetPair, Order, OrderType, OrderV2}
...
class ProofAndAssetPairTestSuite extends MatcherSuiteBase {
  ...
  private val predefAssetPair = earthsUsdPair
  private val aliceEarthsPair  = AssetPair(ByteStr.decodeBase58(aliceAsset).toOption, None)
    ...
    "positive scenarios of order placement" - {
      "set contracts with AssetPairs/all tx fields/true/one proof and then place order" in {
        for (i <- Seq(sc1, sc3, sc4, sc5)) {
          ...
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 500, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)

          val aliceOrd2 = matcherNode
            .placeOrder(aliceAcc, aliceEarthsPair, OrderType.SELL, 500, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(aliceEarthsPair, aliceOrd2, "Accepted", 1.minute)
          ...
          matcherNode.cancelOrder(aliceAcc, aliceEarthsPair, aliceOrd2).status should be("OrderCanceled")
          matcherNode.waitOrderStatus(aliceEarthsPair, aliceOrd2, "Cancelled")
        }
      ...
      "place order and then set contract on AssetPairs/true/all fields/one proof" in {
        for (i <- Seq(sc1, sc3, sc4, sc5)) {
          log.debug(s"contract: $i")
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 500, 2.earths * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)

          val aliceOrd2 = matcherNode
            .placeOrder(aliceAcc, aliceEarthsPair, OrderType.SELL, 500, 2.earths * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(aliceEarthsPair, aliceOrd2, "Accepted", 1.minute)
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 500, 2.earths * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceEarthsPair, OrderType.BUY, 500, 2.earths * Order.PriceConstant, fee = smartMatcherFee, version = 2, 10.minutes)
            .message
            .id

          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Filled", 1.minute)
          matcherNode.waitOrderStatus(aliceEarthsPair, aliceOrd2, "Filled", 1.minute)
          matcherNode.waitOrderStatus(predefAssetPair, bobOrd1, "Filled", 1.minute)
          matcherNode.waitOrderStatus(aliceEarthsPair, bobOrd2, "Filled", 1.minute)
      ...
      "place order and then set contract with many proofs" in {
        ...
        val transferTx = aliceNode.transfer(aliceAcc.address, bobAcc.address, 1000, 0.005.earths, Some(aliceAsset), None, 2)
        nodes.waitForHeightAriseAndTxPresent(transferTx.id)

        for ((i, step) <- Seq(sc5, sc6).zipWithIndex) {
          markup(s"step $step started")

          for (assetP <- Seq(predefAssetPair, aliceEarthsPair)) {
            val currTime = System.currentTimeMillis()
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 500, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceEarthsPair, OrderType.SELL, 500, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
    ...
    "negative scenarios of order placement" - {
      ...
      "place order and then set contract" in {
        for ((contract, i) <- Seq(sc2, sc7, sc8, sc9).zip(Seq(2, 7, 8, 9))) {
          ...
          val aliceOrd1 = matcherNode
            .placeOrder(aliceAcc, predefAssetPair, OrderType.BUY, 100, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          matcherNode.waitOrderStatus(predefAssetPair, aliceOrd1, "Accepted", 1.minute)
          ...
          val bobOrd1 = matcherNode
            .placeOrder(bobAcc, predefAssetPair, OrderType.SELL, 100, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id

          val bobOrd2 = matcherNode
            .placeOrder(bobAcc, aliceEarthsPair, OrderType.BUY, 500, 2.earths * Order.PriceConstant, smartMatcherFee, version = 2, 10.minutes)
            .message
            .id
          ...
          matcherNode.waitOrderStatus(aliceEarthsPair, aliceOrd2, "Cancelled", 1.minute)
          matcherNode.waitOrderStatus(predefAssetPair, bobOrd1, "Accepted", 1.minute)
          matcherNode.waitOrderStatus(aliceEarthsPair, bobOrd2, "Accepted", 1.minute)
          ...
          updatedAliceBalance shouldBe (aliceBalance - 0.014.earths)
          ...
          matcherNode.cancelOrder(bobAcc, aliceEarthsPair, bobOrd2)
          matcherNode.waitOrderStatus(aliceEarthsPair, bobOrd2, "Cancelled")
