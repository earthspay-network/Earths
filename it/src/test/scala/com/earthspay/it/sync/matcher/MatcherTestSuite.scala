...
import com.earthspay.transaction.assets.exchange.{AssetPair, _}
...
class MatcherTestSuite extends MatcherSuiteBase with TableDrivenPropertyChecks {
  ...
  "Check cross ordering between Alice and Bob" - {
    ...
    val order1         = matcherNode.prepareOrder(aliceAcc, aliceEarthsPair, SELL, aliceSellAmount, 2000.earths, version = orderVersion, timeToLive = 2.minutes)
    ...
    "sell order could be placed correctly" - {
      ...
      "and should match with buy order" in {
        ...
        val order2 = matcherNode.placeOrder(bobAcc, aliceEarthsPair, BUY, 200, 2.earths * Order.PriceConstant, matcherFee, orderVersion)
        ...
        matcherNode.orderHistoryByPair(bobAcc, aliceEarthsPair).map(_.id) should contain(order2.message.id)
      ...
      "submitting sell orders should check availability of asset" in {
        ...
        val badOrder = matcherNode.prepareOrder(bobAcc, aliceEarthsPair, SELL, 300, 1900.earths, orderVersion)
        ...
        val order3 = matcherNode.placeOrder(bobAcc, aliceEarthsPair, SELL, 150, 1900.earths, matcherFee, orderVersion)
        matcherNode.waitOrderStatus(aliceEarthsPair, order3.message.id, "Accepted")
      ...
      "buy order should match on few price levels" in {
        ...
        val order4 =
          matcherNode.placeOrder(aliceAcc, aliceEarthsPair, BUY, 350, (21.earths / 10.0 * Order.PriceConstant).toLong, matcherFee, orderVersion)
      ...
      "order could be canceled and resubmitted again" in {
        ...
        val status1 = matcherNode.cancelOrder(aliceAcc, aliceEarthsPair, order1Response.message.id)
        ...
        val order4 = matcherNode.placeOrder(aliceAcc, aliceEarthsPair, SELL, 100, 2000.earths, matcherFee, orderVersion)
        order4.status should be("OrderAccepted")
      ...
      "buy order should execute all open orders and put remaining in order book" in {
        ...
        val order5 = matcherNode.placeOrder(bobAcc, aliceEarthsPair, BUY, 130, 2000.earths, matcherFee, orderVersion)
      ...
      "should consider UTX pool when checking the balance" in {
        ...
        def bobOrder = matcherNode.prepareOrder(bobAcc, bobEarthsPair, SELL, someAssetAmount, 0.005.earths, matcherFee, orderVersion)
        ...
        val order7 = matcherNode.placeOrder(aliceAcc, bobEarthsPair, BUY, someAssetAmount, 0.005.earths, matcherFee, orderVersion)
      ...
      "trader can buy earths for assets with order without having earths" in {
        ...
        def bobOrder =
          matcherNode.prepareOrder(bobAcc, bobEarthsPair, SELL, someAssetAmount, 1.earths, matcherFee, orderVersion)
        ...
        // Cleanup
        matcherNode.cancelOrder(bobAcc, bobEarthsPair, order8.message.id).status should be("OrderCanceled")
      ...
      "market status" in {
        ...
        matcherNode.placeOrder(bobAcc, bobEarthsPair, SELL, askAmount, ask, matcherFee, orderVersion)

        val resp1 = matcherNode.marketStatus(bobEarthsPair)
        ...
        matcherNode.placeOrder(aliceAcc, bobEarthsPair, BUY, bidAmount, bid, matcherFee, orderVersion)

        val resp2 = matcherNode.marketStatus(bobEarthsPair)
