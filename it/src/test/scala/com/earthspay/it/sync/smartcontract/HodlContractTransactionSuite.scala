...
import com.earthspay.lang.v1.compiler.Terms.{CONST_LONG, FUNCTION_CALL}
...
class HodlContractTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("caller deposits earths") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          ...
          p = Some(ContractInvocationTransaction.Payment(1.5.earths, None)),
          timestamp = System.currentTimeMillis(),
          fee = 1.earths
        )
        .explicitGet()
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 1.5.earths)
    val balanceAfter = sender.accountBalances(contract.address)._1

    (balanceAfter - balanceBefore) shouldBe 1.5.earths
  }
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.51.earths))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.earths
        )
  ...
  test("caller can withdraw less than he owns") {
    ...
    val tx =
      ContractInvocationTransaction
        .selfSigned(
          sender = caller,
          contractAddress = contract,
          fc = FUNCTION_CALL(FunctionHeader.User("withdraw"), List(CONST_LONG(1.49.earths))),
          p = None,
          timestamp = System.currentTimeMillis(),
          fee = 1.earths
        )
    ...
    sender.getData(contract.address, caller.address) shouldBe IntegerDataEntry(caller.address, 0.01.earths)
    (balanceAfter - balanceBefore) shouldBe -1.49.earths
  }
}
