...
import com.earthspay.transaction.assets.exchange.{AssetPair, ExchangeTransactionV2, Order}
...
package object smartcontract {
  ...
  def earthsContextScript(dtx: DataTransaction, accountScript: Boolean): String =
    s"""
       | match tx {
       |  case ext : ExchangeTransaction =>
       |    # Earths context
       ...
       |
       |     let balances = assetBalance(ext.sender, unit) > 0 && earthsBalance(ext.sender) != 0
       |
