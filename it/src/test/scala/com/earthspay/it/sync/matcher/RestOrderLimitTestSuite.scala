...
import com.earthspay.it.sync.someAssetAmount
...
class RestOrderLimitTestSuite
  ...
  "Order History REST API methods should have limit for orders in response" in {
    ...
    val active0 = matcher.placeOrder(alice, alicePair, SELL, 1, 15.earths).message.id

    val active1    = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val partial1   = matcher.placeOrder(alice, alicePair, SELL, 2, 9.earths).message.id
    val filled1    = matcher.placeOrder(alice, alicePair, SELL, 1, 8.earths).message.id
    val cancelled1 = matcher.placeOrder(alice, alicePair, SELL, 1, 11.earths).message.id
    val active2    = matcher.placeOrder(alice, bobPair, BUY, 1, 2.earths).message.id
    val filled2    = matcher.placeOrder(alice, bobPair, BUY, 1, 4.earths).message.id
    val partial2   = matcher.placeOrder(alice, bobPair, BUY, 2, 3.earths).message.id
    val cancelled2 = matcher.placeOrder(alice, bobPair, BUY, 1, 2.earths).message.id

    // orders for matching Alice's orders
    matcher.placeOrder(bob, alicePair, BUY, 1, 8.earths).message.id // fill filled1
    matcher.placeOrder(bob, alicePair, BUY, 1, 9.earths).message.id // part fill partial1
    matcher.placeOrder(bob, bobPair, SELL, 1, 4.earths).message.id  // fill filled2
    matcher.placeOrder(bob, bobPair, SELL, 1, 3.earths).message.id  // part fill partial2
    ...
    val active3 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active4 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active5 = matcher.placeOrder(alice, bobPair, BUY, 1, 2.earths).message.id
    val active6 = matcher.placeOrder(alice, bobPair, BUY, 1, 2.earths).message.id
    ...
    val active7  = matcher.placeOrder(alice, alicePair, SELL, 1, 9.earths).message.id
    val active8  = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active9  = matcher.placeOrder(alice, bobPair, BUY, 1, 1.earths).message.id
    val active10 = matcher.placeOrder(alice, bobPair, BUY, 1, 1.earths).message.id
    ...
    matcher.placeOrder(bob, bobPair, SELL, 1, 3.earths).message.id   // fill partial2
    matcher.placeOrder(bob, bobPair, SELL, 2, 2.earths).message.id   // fill active2, active5
    matcher.placeOrder(bob, alicePair, BUY, 2, 9.earths).message.id  // fill partial1, active7
    matcher.placeOrder(bob, alicePair, BUY, 1, 10.earths).message.id // fill active1
    ...
    val active11 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active12 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active13 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active14 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
    val active15 = matcher.placeOrder(alice, alicePair, SELL, 1, 10.earths).message.id
