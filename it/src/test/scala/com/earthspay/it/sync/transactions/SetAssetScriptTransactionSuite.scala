...
import com.earthspay.it.sync.{script, someAssetAmount, _}
...
class SetAssetScriptTransactionSuite extends BaseTransactionSuite {
  ...
  test("cannot transact without having enough earths") {
    val (balance, eff) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.setAssetScript(assetWScript, firstAddress, balance + 1, Some(scriptBase64)), "negative earths balance")
  ...
  test("non-issuer can change script if issuer's account script allows (proof correct)") {
    ...
    val nonIssuerUnsignedTx = SetAssetScriptTransaction(
      ...
      setAssetScriptFee + 0.004.earths,
      System.currentTimeMillis,
      Proofs.empty
    )
