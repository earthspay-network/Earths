package com.earthspay.it.sync.activation
import com.typesafe.config.{Config, ConfigFactory}
import com.earthspay.features.api.NodeFeatureStatus
import com.earthspay.features.{BlockchainFeatureStatus, BlockchainFeatures}
import com.earthspay.it.{Docker, ReportingTestName}
...
object PreActivatedFeaturesTestSuite {
  ...
  private val supportedConfig    = ConfigFactory.parseString(s"""earths {
                                                             ...
                                                             |  miner.quorum = 1
                                                             |}""".stripMargin)
  private val preactivatedConfig = ConfigFactory.parseString(s"""earths {
                                                                |  blockchain.custom.functionality {
                                                                |  feature-check-blocks-period = $votingInterval
                                                                |  pre-activated-features {
                                                                |        1: 0
