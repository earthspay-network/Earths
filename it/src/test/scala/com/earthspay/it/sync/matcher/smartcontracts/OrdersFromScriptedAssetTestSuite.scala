...
import com.earthspay.it.api.SyncHttpApi.NodeExtSync
import com.earthspay.it.api.SyncMatcherHttpApi._
import com.earthspay.it.matcher.MatcherSuiteBase
import com.earthspay.it.sync.createSignedIssueRequest
import com.earthspay.it.sync.matcher.config.MatcherDefaultConfig
...
class OrdersFromScriptedAssetTestSuite extends MatcherSuiteBase {
  ...
  "if counter order has expired then cancel its only, not a submitted order" in {
    val pair = AssetPair.createAssetPair("EARTHS", AllowAssetId).get

    // place expiring counter order
    val counterId =
      matcherNode
        .placeOrder(matcherPk, pair, OrderType.SELL, 2.earths, 10, version = 2, fee = smartTradeFee, timeToLive = 61.seconds)
        .message
        .id
    matcherNode.waitOrderStatus(pair, counterId, "Accepted")
    ...
    val submittedId =
      matcherNode.placeOrder(matcherPk, pair, OrderType.BUY, 2.earths, 10, version = 2, fee = smartTradeFee).message.id
  ...
  "can't execute against unscripted, if the script returns FALSE" in {
    ...
    val setAssetScriptId = matcherNode.setAssetScript(AllowAsset2Id, matcherPk.address, 1.earths, Some(DenyBigAmountScript.bytes().base64)).id
  ...
  "can't execute against scripted, if one script returns FALSE" in {
    ...
    info("update a script")
    val setAssetScriptId = matcherNode.setAssetScript(AllowAsset3Id, matcherPk.address, 1.earths, Some(DenyBigAmountScript.bytes().base64)).id
    matcherNode.waitForTransaction(setAssetScriptId)
