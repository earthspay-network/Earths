...
class OrderBookTestSuite extends MatcherSuiteBase {
  ...
  "When delete order book" - {
    ...
    val buyOrderForAnotherPair = matcherNode.placeOrder(aliceAcc, wctEarthsPair, BUY, amount, price, matcherFee).message.id
    val sellOrderForAnotherPair =
      matcherNode.placeOrder(bobAcc, wctEarthsPair, SELL, amount, 2 * price, matcherFee).message.id
