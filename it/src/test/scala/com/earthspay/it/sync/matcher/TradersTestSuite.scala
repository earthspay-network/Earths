...
class TradersTestSuite extends MatcherSuiteBase {
  ...
  "Verifications of tricky ordering cases" - {
    ...
    "owner moves assets/earths to another account and order become an invalid" - {
      ...
      "order with assets" - {
        "moved assets, insufficient assets" in {
          ...
          matcherNode.waitForTransaction(transferId) // 5000 earths are rest

          withClue(s"The newest order '$newestOrderId' was cancelled") {
            matcherNode.waitOrderStatus(bobEarthsPair, newestOrderId, "Cancelled")
          }
          withClue(s"The oldest order '$oldestOrderId' is still active") {
            matcherNode.orderStatus(oldestOrderId, bobEarthsPair).status shouldBe "Accepted"
          }
      ...
      "order with earths" - {
        "leased earths, insufficient fee for one ExchangeTransaction" in {
          ...
          val oldestOrderId = bobPlacesEarthOrder(bobEarthsPair, 1, 10.earths * Order.PriceConstant)
          val newestOrderId = bobPlacesEarthOrder(bobEarthsPair, 1, 10.earths * Order.PriceConstant)

          //      waitForOrderStatus(matcherNode, bobAssetIdRaw, id, "Accepted")
          val leaseAmount = bobBalance - exTxFee - 10.earths - matcherFee
          val leaseId     = bobNode.lease(bobAcc.address, aliceAcc.address, leaseAmount, exTxFee, 2).id
          matcherNode.waitForTransaction(leaseId)
          ...
          // Cleanup
          matcherNode.cancelOrder(bobAcc, bobEarthsPair, oldestOrderId)
          matcherNode.waitOrderStatus(twoAssetsPair, oldestOrderId, "Cancelled")
        ...
        "leased earths, insufficient earths" in {
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
          val price      = 1.earths
          val order2     = bobPlacesEarthOrder(bobEarthsPair, 1, price * Order.PriceConstant)
        ...
        "moved earths, insufficient fee" in {
          ...
          val bobBalance = matcherNode.accountBalances(bobAcc.address)._1
          val price      = exTxFee / 2
          val order3     = bobPlacesEarthOrder(bobEarthsPair, 1, price * Order.PriceConstant)
  ...
  def bobPlacesEarthOrder(assetPair: AssetPair, amount: Long, price: Price): String = {
