...
import com.earthspay.it.matcher.{MatcherCommand, MatcherState, MatcherSuiteBase}
...
class MultipleMatchersTestSuite extends MatcherSuiteBase {
  private def configOverrides = ConfigFactory.parseString("""earths.matcher {
      |  price-assets = ["EARTHS"]
      |  snapshots-interval = 51
      |}""".stripMargin)

  private def matcher1NodeConfig = Default.last
  private def matcher2NodeConfig = ConfigFactory.parseString("""earths.network.node-name = node11
