...
class RIDEFuncSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("assetBalance() verification") {
    ...
    sender.transfer(acc0.address, newAddress, 10.earths, minFee, waitForTx = true)
    ...
    assertBadRequestAndResponse(
      sender.signedBroadcast(
        TransferTransactionV2
          .selfSigned(None, pkNewAddress, pkNewAddress, 1.earths, System.currentTimeMillis(), None, smartMinFee, Array())
          .explicitGet()
          .json()),
    ...
    val udpatedScript =
      s"""
         |match tx {
         |  case tx : SetScriptTransaction => true
         |  case other => assetBalance(tx.sender, base58'$asset') >= 900000000 && earthsBalance(tx.sender) >500000000
         |}
      """.stripMargin
