...
import com.earthspay.transaction.{CreateAliasTransaction, DataTransaction, GenesisTransaction, PaymentTransaction}
import com.earthspay.transaction.assets.{BurnTransaction, IssueTransaction, ReissueTransaction, SponsorFeeTransaction}
import com.earthspay.transaction.assets.exchange.{AssetPair, Order, _}
...
import com.earthspay.transaction.transfer.MassTransferTransaction.Transfer
import com.earthspay.transaction.transfer.{MassTransferTransaction, TransferTransaction}
...
class SignAndBroadcastApiSuite extends BaseTransactionSuite with NTPTime {
  test("height should always be reported for transactions") {
    val txId = sender.transfer(firstAddress, secondAddress, 1.earths, fee = minFee).id
  ...
  test("/transactions/sign should produce issue/reissue/burn/transfer transactions that are good for /transactions/broadcast") {
    for (v <- supportedVersions) {
      ...
      signBroadcastAndCalcFee(
        Json.obj("type" -> ReissueTransaction.typeId, "quantity" -> 200.earths, "assetId" -> issueId, "sender" -> firstAddress, "reissuable" -> false),
        usesProofs = isProof,
        version = v
      )
      ...
      signBroadcastAndCalcFee(Json.obj("type" -> BurnTransaction.typeId, "quantity" -> 100.earths, "assetId" -> issueId, "sender" -> firstAddress),
                              usesProofs = isProof,
                              version = v)
  ...
  test("/transactions/sign should produce mass transfer transaction that is good for /transactions/broadcast") {
    signBroadcastAndCalcFee(
      Json.obj(
        ...
        "version"    -> 1,
        "sender"     -> firstAddress,
        "transfers"  -> Json.toJson(Seq(Transfer(secondAddress, 1.earths), Transfer(thirdAddress, 2.earths))),
  ...
  test("/transactions/broadcast should produce ExchangeTransaction with custom asset") {
    ...
    for ((o1ver, o2ver, tver) <- Seq(
      ...
      val buyAmount           = 2
      val sellAmount          = 3
      val assetPair           = AssetPair.createAssetPair("EARTHS", issueTx).get
