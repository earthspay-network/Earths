...
class CancelOrderTestSuite extends MatcherSuiteBase {
  ...
  "Order can be canceled" - {
    "by sender" in {
      val orderId = matcherNode.placeOrder(bobAcc, earthsUsdPair, OrderType.SELL, 100.earths, 800, matcherFee).message.id
      ...
      matcherNode.orderHistoryByPair(bobAcc, earthsUsdPair).collectFirst {
    ...
    "with API key" in {
      ...
      matcherNode.orderHistoryByPair(bobAcc, earthsUsdPair).filter(_.id == orderId).head.status shouldBe "Cancelled"
  ...
  "Cancel is rejected" - {
    "when request sender is not the sender of and order" in {
      ...
      // Cleanup
      matcherNode.cancelOrder(bobAcc, earthsUsdPair, orderId)
      matcherNode.waitOrderStatus(earthsUsdPair, orderId, "Cancelled")
    }
  }
    ...
    "works for" - {
      "all orders placed by an address" in {
        ...
        val usdOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, earthsUsdPair, OrderType.SELL, 100.earths + i, 400, matcherFee).message.id
        }

        matcherNode.assetBalance(bobAcc.toAddress.stringRepr, BtcId.base58)

        val btcOrderIds = 1 to 5 map { i =>
          matcherNode.placeOrder(bobAcc, earthsBtcPair, OrderType.BUY, 100.earths + i, 400, matcherFee).message.id
        }
      ...
      "a pair" in {
        ...
        matcherNode.cancelOrdersForPair(bobAcc, earthsBtcPair)
