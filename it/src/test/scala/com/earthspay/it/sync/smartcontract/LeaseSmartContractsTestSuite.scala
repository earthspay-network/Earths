...
import com.earthspay.it.sync.{minFee, setScriptFee, transferAmount}
...
import com.earthspay.transaction.lease.{LeaseCancelTransactionV2, LeaseTransactionV2}
...
class LeaseSmartContractsTestSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract, make leasing and cancel leasing") {
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (minFee + setScriptFee + 0.2.earths),
                            eff1 + 9 * transferAmount - (minFee + setScriptFee + 0.2.earths))
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.earths),
                            eff1 + 10 * transferAmount - (2 * minFee + setScriptFee + 2 * 0.2.earths))
