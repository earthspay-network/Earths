...
class LeaseSmartContractsTestSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("set contract, make leasing and cancel leasing") {
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (2 * minFee + 0.2.earths),
                            eff1 + 9 * transferAmount - (2 * minFee + 0.2.earths))
    ...
    notMiner.assertBalances(firstAddress,
                            balance1 + 10 * transferAmount - (3 * minFee + 2 * 0.2.earths),
                            eff1 + 10 * transferAmount - (3 * minFee + 2 * 0.2.earths))
