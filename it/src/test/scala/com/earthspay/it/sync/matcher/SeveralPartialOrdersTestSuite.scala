...
import com.earthspay.it.sync.matcherFee
...
class SeveralPartialOrdersTestSuite
  ...
  "Alice and Bob trade EARTHS-USD" - {
    ...
    "place two submitted orders and one counter" in {
      ...
      val aliceOrder1  = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, price, buyOrderAmount)
      ...
      val aliceOrder2   = matcherNode.prepareOrder(aliceNode, earthsUsdPair, OrderType.BUY, price, buyOrderAmount)
      val aliceOrder2Id = matcherNode.placeOrder(aliceOrder2).message.id
      matcherNode.waitOrderStatus(earthsUsdPair, aliceOrder2Id, "Filled", 1.minute)

      matcherNode.waitOrderStatus(earthsUsdPair, bobOrder1Id, "Filled", 1.minute)
      ...
      val orderBook1 = matcherNode.orderBook(earthsUsdPair)
      orderBook1.asks shouldBe empty
      orderBook1.bids shouldBe empty

      val bobOrder2   = matcherNode.prepareOrder(bobNode, earthsUsdPair, OrderType.SELL, price, sellOrderAmount)
      val bobOrder2Id = matcherNode.placeOrder(bobOrder2).message.id
      matcherNode.waitOrderStatus(earthsUsdPair, bobOrder2Id, "Accepted", 1.minute)

      val orderBook2 = matcherNode.orderBook(earthsUsdPair)
      orderBook2.asks shouldBe List(LevelResponse(bobOrder2.price, bobOrder2.amount))
      orderBook2.bids shouldBe empty

      matcherNode.cancelOrder(bobNode, earthsUsdPair, Some(bobOrder2Id))
      nodes.waitForHeightArise()
    ...
    "place one submitted orders and two counter" in {
      ...
      matcherNode.waitOrderStatus(earthsUsdPair, aliceOrder1Id, "Filled", 1.minute)
      matcherNode.waitOrderStatus(earthsUsdPair, aliceOrder2Id, "Filled", 1.minute)
      matcherNode.waitOrderStatus(earthsUsdPair, bobOrder1Id, "Filled", 1.minute)
