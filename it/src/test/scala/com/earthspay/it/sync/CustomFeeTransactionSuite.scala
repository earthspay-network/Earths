...
import com.earthspay.state.{EitherExt2, Sponsorship}
...
class CustomFeeTransactionSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("make transfer with sponsored asset") {
    ...
    val sponsorship = Sponsorship.toEarths(transferFee, assetToken)
...
object CustomFeeTransactionSuite {
  ...
  private val minerConfig = ConfigFactory.parseString(s"""
      | earths.fees.transfer.$assetId = 100000
      | earths.blockchain.custom.functionality {
  ...
  private val notMinerConfig = ConfigFactory.parseString("earths.miner.enable=no").withFallback(minerConfig)
