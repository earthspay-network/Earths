...
class LeasingTransactionsSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("leasing earths decreases lessor's eff.b. and increases lessee's eff.b.; lessor pays fee") {
  ...
  test("can not make leasing without having enough balance") {
    ...
    assertBadRequestAndResponse(sender.lease(secondAddress, firstAddress, balance2 + 1.earths, minFee), "Reason: Cannot lease more than own")
  ...
  test("can not make leasing without having enough earths for fee") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    val (balance2, eff2) = notMiner.accountBalances(secondAddress)
  ...
  test("lease cancellation reverts eff.b. changes; lessor pays fee for both lease and cancellation") {
    import com.earthspay.transaction.lease.LeaseTransaction.Status._
  ...
  test("can not make leasing without having enough your earths to self") {
    val (balance1, eff1) = notMiner.accountBalances(firstAddress)
    assertBadRequestAndResponse(sender.lease(firstAddress, firstAddress, balance1 + 1.earths, minFee), "Transaction to yourself")
    nodes.waitForHeightArise()
