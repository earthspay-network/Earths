...
class MatcherMassOrdersTestSuite extends MatcherSuiteBase {
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceOrderIdFill = matcherNode
      .placeOrder(aliceAcc, aliceSecondEarthsPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 10.minutes)
      .message
      .id
    ...
    val aliceOrderToCancelId =
      matcherNode
        .placeOrder(aliceAcc, aliceSecondEarthsPair, OrderType.SELL, 3, Order.PriceConstant, matcherFee, orderVersion, 70.seconds)
        .message
        .id

    val aliceActiveOrderId = matcherNode
      .placeOrder(aliceAcc, aliceSecondEarthsPair, OrderType.SELL, 3, Order.PriceConstant + 100000000, matcherFee, orderVersion, 10.minutes)
      .message
      .id

    matcherNode.cancelOrder(aliceAcc, aliceSecondEarthsPair, aliceOrderToCancelId) // TODO: remove this line in DEX-160
    ...
    ordersRequestsGen(2, bobAcc, aliceSecondEarthsPair, OrderType.BUY, 2)

    //check orders after filling
    matcherNode.waitOrderStatus(aliceSecondEarthsPair, alicePartialOrderId, "PartiallyFilled")

    orderStatus(aliceAcc, aliceSecondEarthsPair, aliceOrderIdFill, "Filled")
    orderStatus(aliceAcc, aliceSecondEarthsPair, alicePartialOrderId, "PartiallyFilled")
    ...
    "Mass orders creation with random lifetime. Active orders still in list" in {
      ...
      ordersRequestsGen(orderLimit + 1, aliceAcc, aliceEarthsPair, OrderType.SELL, 3)

      //wait for some orders cancelled
      Thread.sleep(5000)
      val bobsOrderIds = ordersRequestsGen(orderLimit + 1, bobAcc, aliceEarthsPair, OrderType.BUY, 2)
      Thread.sleep(5000)
      ...
      matcherNode.orderHistoryByPair(bobAcc, aliceEarthsPair).map(_.id) should equal(bobsOrderIds.drop(1).reverse)
    }
    ...
    "check order history orders count after fill" in {
      ...
      val aliceOrderHistoryByPair = matcherNode.orderHistoryByPair(aliceAcc, aliceEarthsPair)
