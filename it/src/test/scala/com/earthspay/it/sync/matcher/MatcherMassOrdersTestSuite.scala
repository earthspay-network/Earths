...
class MatcherMassOrdersTestSuite
  ...
  "Create orders with statuses FILL, PARTIAL, CANCELLED, ACTIVE" - {
    ...
    val aliceOrderIdFill = matcherNode
      .placeOrder(aliceNode, aliceSecondEarthsPair, OrderType.SELL, Order.PriceConstant, 3, 10.minutes)
      .message
      .id
    ...
    val aliceOrderToCancelId =
      matcherNode
        .placeOrder(aliceNode, aliceSecondEarthsPair, OrderType.SELL, Order.PriceConstant, 3, 70.seconds)
        .message
        .id

    val aliceActiveOrderId = matcherNode
      .placeOrder(aliceNode, aliceSecondEarthsPair, OrderType.SELL, Order.PriceConstant + 1, 3, 10.minutes)
      .message
      .id
