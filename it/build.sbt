...
inTask(docker)(
  Seq(
    dockerfile := {
      ...
      val startEarths     = sourceDirectory.value / "container" / "start-earths.sh"
      ...
      new Dockerfile {
        ...
        if (withAspectJ) run("wget", "--quiet", aspectjAgentUrl, "-O", "/opt/earths/aspectjweaver.jar")

        add((assembly in LocalProject("node")).value, "/opt/earths/earths.jar")
        add(Seq(configTemplate, startEarths), "/opt/earths/")
        runShell("chmod", "+x", "/opt/earths/start-earths.sh")
        entryPoint("/opt/earths/start-earths.sh")
        expose(10001)
      }
    },
        ...
        Tests.SubProcess(
          ForkOptions(
            ...
            runJVMOptions = Vector(
              ...
              "-Dearths.it.logging.appender=FILE",
              s"-Dearths.it.logging.dir=${logDirectoryValue / suite.name.replaceAll("""(\w)\w*\.""", "$1.")}"
            ) ++ javaOptionsValue,
            connectInput = false,
